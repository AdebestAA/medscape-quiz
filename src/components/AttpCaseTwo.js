...
const AttpCaseTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAttpCaseTwo($id: ID!) {
    deleteAttpCaseTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAttpCaseTwo($data: AttpCaseTwoCreateInput!) {
    createAttpCaseTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAttpCaseTwo($id: ID!) {
    publishAttpCaseTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAttpCaseTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  ...
  attpCaseTwos {
    ...
    if (existingData.attpCaseTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.attpCaseTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAttpCaseTwo.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchAttpCaseTwoData = async () => {
      const query = gql`
        query {
            attpCaseTwos {
            id
      ...
      try {
        ...
        console.log(getData.attpCaseTwos[0]);
        setQuestionsToDisplay(getData.attpCaseTwos)
