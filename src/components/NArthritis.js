...
const NArthriti = ({dataFromTypeForm}) => {
...
overwriteAndPublishQuest(dataFromTypeForm);
 ...
 mutation deleteNArthriti($id: ID!) {
    deleteNArthriti(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createNArthriti($data: NArthritiCreateInput!) {
    createNArthriti(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishNArthriti($id: ID!) {
    publishNArthriti(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createNArthriti.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   nArthritis {
    ...
    if (existingData.nArthritis.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.nArthritis[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishNArthriti.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchNArthritiData = async () => {
      const query = gql`
        query {
            nArthritis {
            id
      ...
      try {
        ...
        console.log(getData.nArthritis[0]);
        setQuestionsToDisplay(getData.nArthritis)
