...
const PolycythemiaVeraMpnsWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deletePolycythemiaVeraMpnsWebhook($id: ID!) {
    deletePolycythemiaVeraMpnsWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createPolycythemiaVeraMpnsWebhook($data: PolycythemiaVeraMpnsWebhookCreateInput!) {
    createPolycythemiaVeraMpnsWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishPolycythemiaVeraMpnsWebhook($id: ID!) {
    publishPolycythemiaVeraMpnsWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createPolycythemiaVeraMpnsWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingPolycythemiaVeraMpnsWebhooks {
   polycythemiaVeraMpnsWebhooks {
    ...
    if (existingData.polycythemiaVeraMpnsWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.polycythemiaVeraMpnsWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishPolycythemiaVeraMpnsWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchPolycythemiaVeraMpnsWebhookData = async () => {
      const query = gql`
        query {
            polycythemiaVeraMpnsWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.polycythemiaVeraMpnsWebhooks[0]);
        setQuestionsToDisplay(getData.polycythemiaVeraMpnsWebhooks)
   ...
   fetchPolycythemiaVeraMpnsWebhookData();
  }, [dataPosted]);
...
export default PolycythemiaVeraMpnsWebhooks