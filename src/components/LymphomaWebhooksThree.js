...
const LymphomaWebhooksThree = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLymphomaWebhooksThree($id: ID!) {
    deleteLymphomaWebhooksThree(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLymphomaWebhooksThree($data: LymphomaWebhooksThreeCreateInput!) {
    createLymphomaWebhooksThree(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLymphomaWebhooksThree($id: ID!) {
    publishLymphomaWebhooksThree(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLymphomaWebhooksThree.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingLymphomaWebhooksThrees {
   lymphomaWebhooksThrees {
    id
    placeholder
    }
    }
    ...
    if (existingData.lymphomaWebhooksThrees.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.lymphomaWebhooksThrees[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLymphomaWebhooksThree.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchLymphomaWebhooksThreeData = async () => {
      const query = gql`
        query {
            lymphomaWebhooksThrees {
            id
      ...
      try {
        ...
        console.log(getData.lymphomaWebhooksThrees[0]);
        setQuestionsToDisplay(getData.lymphomaWebhooksThrees)
   ...
   fetchLymphomaWebhooksThreeData();
  }, [dataPosted]);
...
export default LymphomaWebhooksThree