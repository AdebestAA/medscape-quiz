...
const NDiarrhea = ({dataFromTypeForm}) => {
...
overwriteAndPublishQuest(dataFromTypeForm);
 ...
 mutation deleteNDiarrhea($id: ID!) {
    deleteNDiarrhea(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createNDiarrhea($data: NDiarrheaCreateInput!) {
    createNDiarrhea(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishNDiarrhea($id: ID!) {
    publishNDiarrhea(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createNDiarrhea.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   nDiarrheas {
    ...
    if (existingData.nDiarrheas.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.nDiarrheas[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishNDiarrhea.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchNDiarrheaData = async () => {
      const query = gql`
        query {
            nDiarrheas {
            id
      ...
      try {
        ...
        console.log(getData.nDiarrheas[0]);
        setQuestionsToDisplay(getData.nDiarrheas)
