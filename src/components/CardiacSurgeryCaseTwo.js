...
const CardiacSurgeryCaseTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCardiacSurgeryCaseTwo($id: ID!) {
    deleteCardiacSurgeryCaseTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCardiacSurgeryCaseTwo($data: CardiacSurgeryCaseTwoCreateInput!) {
    createCardiacSurgeryCaseTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCardiacSurgeryCaseTwo($id: ID!) {
    publishCardiacSurgeryCaseTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createPnhCaseTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCardiacSurgeryCaseTwos {
  cardiacSurgeryCaseTwos {
    id
    placeholder
    }
    }
    ...
    if (existingData.cardiacSurgeryCaseTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.cardiacSurgeryCaseTwos[0].id;
  ...
  placeholder: newData
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCardiacSurgeryCaseTwo.id;
    console.log('New quest created:', newQuest);
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCardiacSurgeryCaseTwo.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCardiacSurgeryCaseTwoData = async () => {
      const query = gql`
        query {
            cardiacSurgeryCaseTwos {
            id
      ...
      try {
        ...
        console.log(getData.cardiacSurgeryCaseTwos[0]);
        setQuestionsToDisplay(getData.cardiacSurgeryCaseTwos)
   ...
   fetchCardiacSurgeryCaseTwoData();
  }, [dataPosted]);
...
export default CardiacSurgeryCaseTwo