...
const CovidNineteen = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCovidNineteen($id: ID!) {
    deleteCovidNineteen(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCovidNineteen($data: CovidNineteenCreateInput!) {
    createCovidNineteen(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCovidNineteen($id: ID!) {
    publishCovidNineteen(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCovidNineteen.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCovidNineteens {
   covidNineteens {
    id
    placeholder
    }
    ...
    if (existingData.covidNineteens.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.covidNineteens[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCovidNineteen.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCovidNineteenData = async () => {
      const query = gql`
        query {
            covidNineteens {
            id
      ...
      try {
        ...
        console.log(getData.covidNineteens[0]);
        setQuestionsToDisplay(getData.covidNineteens)
