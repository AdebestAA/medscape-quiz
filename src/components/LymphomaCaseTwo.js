...
const LymphomaCaseTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLymphomaCaseTwo($id: ID!) {
    deleteLymphomaCaseTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLymphomaCaseTwo($data: LymphomaCaseTwoCreateInput!) {
    createLymphomaCaseTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLymphomaCaseTwo($id: ID!) {
    publishLymphomaCaseTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLymphomaCaseTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingLymphomaCaseTwos {
   lymphomaCaseTwos {
    id
    placeholder
    }
    ...
    if (existingData.lymphomaCaseTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.lymphomaCaseTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLymphomaCaseTwo.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchLymphomaCaseTwoData = async () => {
      const query = gql`
        query {
            lymphomaCaseTwos {
            id
      ...
      try {
        ...
        console.log(getData.lymphomaCaseTwos[0]);
        setQuestionsToDisplay(getData.lymphomaCaseTwos)
...
export default LymphomaCaseTwo