...
const Hypothyroidism = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteHypothyroidism($id: ID!) {
    deleteHypothyroidism(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHypothyroidism($data: HypothyroidismCreateInput!) {
    createHypothyroidism(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishHypothyroidism($id: ID!) {
    publishHypothyroidism(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHypothyroidism.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingHypothyroidisms {
   hypothyroidisms {
    id
    placeholder
    }
    ...
    if (existingData.hypothyroidisms.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.hypothyroidisms[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHypothyroidism.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchHypothyroidismData = async () => {
      const query = gql`
        query {
            hypothyroidisms {
            id
      ...
      try {
        ...
        console.log(getData.hypothyroidisms[0]);
        setQuestionsToDisplay(getData.hypothyroidisms)
