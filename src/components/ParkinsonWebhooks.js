...
const ParkinsonWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteParkinsonWebhook($id: ID!) {
    deleteParkinsonWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createParkinsonWebhook($data: ParkinsonWebhookCreateInput!) {
    createParkinsonWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishParkinsonWebhook($id: ID!) {
    publishParkinsonWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createParkinsonWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingParkinsonWebhooks {
   parkinsonWebhooks {
    id
    placeholder
    }
    ...
    if (existingData.parkinsonWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.parkinsonWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishParkinsonWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchParkinsonWebhookData = async () => {
      const query = gql`
        query {
            parkinsonWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.parkinsonWebhooks[0]);
        setQuestionsToDisplay(getData.parkinsonWebhooks)
...
export default ParkinsonWebhooks