...
const MultipleSclerosisEnd = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteMultipleSclerosisEnd($id: ID!) {
    deleteMultipleSclerosisEnd(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createMultipleSclerosisEnd($data: MultipleSclerosisEndCreateInput!) {
    createMultipleSclerosisEnd(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishMultipleSclerosisEnd($id: ID!) {
    publishMultipleSclerosisEnd(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createMultipleSclerosisEnd.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingMultipleSclerosisEnds {
  multipleSclerosisEnds {
    id
    placeholder
    }
    }
    ...
    if (existingData.multipleSclerosisEnds.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.multipleSclerosisEnds[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishMultipleSclerosisEnd.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchMultipleSclerosisEndData = async () => {
      const query = gql`
        query {
            multipleSclerosisEnds {
            id
      ...
      try {
        ...
        console.log(getData.multipleSclerosisEnds[0]);
        setQuestionsToDisplay(getData.multipleSclerosisEnds)
...
export default MultipleSclerosisEnd