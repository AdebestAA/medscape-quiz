...
const BladderCancerWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteBladderCancerWebhook($id: ID!) {
    deleteBladderCancerWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createBladderCancerWebhook($data: BladderCancerWebhookCreateInput!) {
    createBladderCancerWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishBladderCancerWebhookn($id: ID!) {
    publishBladderCancerWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createBladderCancerWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingBladderCancerWebhooks {
   bladderCancerWebhooks {
    ...
    if (existingData.bladderCancerWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.bladderCancerWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishBladderCancerWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchBladderCancerWebhookData = async () => {
      const query = gql`
        query {
            bladderCancerWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.bladderCancerWebhooks[0]);
        setQuestionsToDisplay(getData.bladderCancerWebhooks)
...
export default BladderCancerWebhooks