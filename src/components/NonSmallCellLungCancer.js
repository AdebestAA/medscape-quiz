...
const NonSmallCellLungCancer = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteNonSmallCellLungCancer($id: ID!) {
    deleteNonSmallCellLungCancer(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createNonSmallCellLungCancer($data: NonSmallCellLungCancerCreateInput!) {
    createNonSmallCellLungCancer(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishNonSmallCellLungCancer($id: ID!) {
    publishNonSmallCellLungCancer(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createNonSmallCellLungCancer.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingNonSmallCellLungCancers {
   nonSmallCellLungCancers {
    ...
    if (existingData.nonSmallCellLungCancers.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.nonSmallCellLungCancers[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishNonSmallCellLungCancer.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchNonSmallCellLungCancerData = async () => {
      const query = gql`
        query {
            nonSmallCellLungCancers {
            id
      ...
      try {
        ...
        console.log(getData.nonSmallCellLungCancers[0]);
        setQuestionsToDisplay(getData.nonSmallCellLungCancers)
   ...
   fetchNonSmallCellLungCancerData();
  }, [dataPosted]);
...
export default NonSmallCellLungCancer