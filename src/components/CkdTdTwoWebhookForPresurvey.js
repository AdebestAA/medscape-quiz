...
const CkdTdTwoWebhookForPresurvey = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCkdTdTwoWebhookForPresurvey($id: ID!) {
    deleteCkdTdTwoWebhookForPresurvey(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCkdTdTwoWebhookForPresurvey($data: CkdTdTwoWebhookForPresurveyCreateInput!) {
    createCkdTdTwoWebhookForPresurvey(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCkdTdTwoWebhookForPresurvey($id: ID!) {
    publishCkdTdTwoWebhookForPresurvey(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCkdTdTwoWebhookForPresurvey.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCkdTdTwoWebhookForPresurveys {
   ckdTdTwoWebhookForPresurveys {
    id
    placeholder
    }
    }
    ...
    if (existingData.ckdTdTwoWebhookForPresurveys.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.ckdTdTwoWebhookForPresurveys[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCkdTdTwoWebhookForPresurvey.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCkdTdTwoWebhookForPresurveyData = async () => {
      const query = gql`
        query {
           ckdTdTwoWebhookForPresurveys {
            id
      ...
      try {
        ...
        console.log(getData.ckdTdTwoWebhookForPresurveys[0]);
        setQuestionsToDisplay(getData.ckdTdTwoWebhookForPresurveys)
   ...
   fetchCkdTdTwoWebhookForPresurveyData();
  }, [dataPosted]);
...
export default CkdTdTwoWebhookForPresurvey