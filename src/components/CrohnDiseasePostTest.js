...
const CrohnDiseasePostTest = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCrohnDiseasePostTest($id: ID!) {
    deleteCrohnDiseasePostTest(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCrohnDiseasePostTest($data: CrohnDiseasePostTestCreateInput!) {
    createCrohnDiseasePostTest(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCrohnDiseasePostTest($id: ID!) {
    publishCrohnDiseasePostTest(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCrohnDiseasePostTest.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCrohnDiseasePostTests {
   crohnDiseasePostTests {
    id
    placeholder
    }
    }
    ...
    if (existingData.crohnDiseasePostTests.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.crohnDiseasePostTests[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCrohnDiseasePostTest.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCrohnDiseasePostTestData = async () => {
      const query = gql`
        query {
            crohnDiseasePostTests {
            id
      ...
      try {
        ...
        console.log(getData.crohnDiseasePostTests[0]);
        setQuestionsToDisplay(getData.crohnDiseasePostTests)
...
export default CrohnDiseasePostTest
