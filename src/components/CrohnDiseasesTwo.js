...
const CrohnDiseasesTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCrohnDiseasesTwo($id: ID!) {
    deleteCrohnDiseasesTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCrohnDiseasesTwo($data: CrohnDiseasesTwoCreateInput!) {
    createCrohnDiseasesTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCrohnDiseasesTwo($id: ID!) {
    publishCrohnDiseasesTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCrohnDiseasesTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCrohnDiseasesTwos {
   crohnDiseasesTwos {
    id
    placeholder
    }
    ...
    if (existingData.crohnDiseasesTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.crohnDiseasesTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCrohnDiseasesTwo.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCrohnDiseasesTwoData = async () => {
      const query = gql`
        query {
            crohnDiseasesTwos {
            id
      ...
      try {
        ...
        console.log(getData.crohnDiseasesTwos[0]);
        setQuestionsToDisplay(getData.crohnDiseasesTwos)
...
export default CrohnDiseasesTwo