...
const HypoalbuminemiaOne = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteHypoalbuminemiaOne($id: ID!) {
    deleteHypoalbuminemiaOne(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHypoalbuminemiaOne($data: HypoalbuminemiaOneCreateInput!) {
    createHypoalbuminemiaOne(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishHypoalbuminemiaOne($id: ID!) {
    publishHypoalbuminemiaOne(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHypoalbuminemiaOne.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingHypoalbuminemiaOnes {
   hypoalbuminemiaOnes {
    id
    placeholder
    }
    ...
    if (existingData.hypoalbuminemiaOnes.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.hypoalbuminemiaOnes[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHypoalbuminemiaOne.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchHypoalbuminemiaOneData = async () => {
      const query = gql`
        query {
            hypoalbuminemiaOnes {
            id
      ...
      try {
        ...
        console.log(getData.hypoalbuminemiaOnes[0]);
        setQuestionsToDisplay(getData.hypoalbuminemiaOnes)
...
export default HypoalbuminemiaOne
