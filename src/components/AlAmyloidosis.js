...
const AlAmyloidosis = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAlAmyloidosis($id: ID!) {
    deleteAlAmyloidosis(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAlAmyloidosis($data: AlAmyloidosisCreateInput!) {
    createAlAmyloidosis(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAlAmyloidosis($id: ID!) {
    publishAlAmyloidosis(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAlAmyloidosis.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingAlAmyloidosiss {
   alAmyloidosiss {
    id
    placeholder
    }
    ...
    if (existingData.alAmyloidosiss.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.alAmyloidosiss[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAlAmyloidosis.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchAlAmyloidosisData = async () => {
      const query = gql`
        query {
            alAmyloidosiss {
            id
      ...
      try {
        ...
        console.log(getData.alAmyloidosiss[0]);
        setQuestionsToDisplay(getData.alAmyloidosiss)
