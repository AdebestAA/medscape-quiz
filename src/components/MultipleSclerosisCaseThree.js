...
const MultipleSclerosisCaseThree = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteMultipleSclerosisCaseThree($id: ID!) {
    deleteMultipleSclerosisCaseThree(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createMultipleSclerosisCaseThree($data: MultipleSclerosisCaseThreeCreateInput!) {
    createMultipleSclerosisCaseThree(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishMultipleSclerosisCaseThree($id: ID!) {
    publishMultipleSclerosisCaseThree(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createMultipleSclerosisCaseThree.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingMultipleSclerosisCaseThrees {
  multipleSclerosisCaseThrees {
    id
    placeholder
    }
    }
    ...
    if (existingData.multipleSclerosisCaseThrees.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.multipleSclerosisCaseThrees[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishMultipleSclerosisCaseThree.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchMultipleSclerosisCaseThreeData = async () => {
      const query = gql`
        query {
            multipleSclerosisCaseThrees {
            id
      ...
      try {
        ...
        console.log(getData.multipleSclerosisCaseThrees[0]);
        setQuestionsToDisplay(getData.multipleSclerosisCaseThrees)
   ...
   fetchMultipleSclerosisCaseThreeData();
  }, [dataPosted]);
...
export default MultipleSclerosisCaseThree