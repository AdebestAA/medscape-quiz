...
const CytomegalovirusPostTestPartB = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCytomegalovirusPostTestPartB($id: ID!) {
    deleteCytomegalovirusPostTestPartB(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCytomegalovirusPostTestPartB($data: CytomegalovirusPostTestPartBCreateInput!) {
    createCytomegalovirusPostTestPartB(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCytomegalovirusPostTestPartB($id: ID!) {
    publishCytomegalovirusPostTestPartB(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCytomegalovirusPostTestPartB.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCytomegalovirusPostTestPartBs {
  cytomegalovirusPostTestPartBs {
    id
    placeholder
    }
    }
    ...
    if (existingData.cytomegalovirusPostTestPartBs.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.cytomegalovirusPostTestPartBs[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCytomegalovirusPostTestPartB.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCytomegalovirusPostTestPartBData = async () => {
      const query = gql`
        query {
            cytomegalovirusPostTestPartBs {
            id
      ...
      try {
        ...
        console.log(getData.cytomegalovirusPostTestPartBs[0]);
        setQuestionsToDisplay(getData.cytomegalovirusPostTestPartBs)
   ...
   fetchCytomegalovirusPostTestPartBData();
  }, [dataPosted]);
...
export default CytomegalovirusPostTestPartB