...
const IttpFull = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteIttpFull($id: ID!) {
    deleteIttpFull(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createIttpFull($data: IttpFullCreateInput!) {
    createIttpFull(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishIttpFull($id: ID!) {
    publishIttpFull(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createIttpFull.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   ittpFulls {
    ...
    if (existingData.ittpFulls.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.ittpFulls[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishIttpFull.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchIttpFullData = async () => {
      ...
      try {
        ...
        console.log(getData.ittpFulls[0]);
        setQuestionsToDisplay(getData.ittpFulls)
