...
const AtopicDermatitis = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAtopicDermatitis($id: ID!) {
    deleteAtopicDermatitis(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAtopicDermatitis($data: AtopicDermatitisCreateInput!) {
    createAtopicDermatitis(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAtopicDermatitis($id: ID!) {
    publishAtopicDermatitis(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAtopicDermatitis.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingAtopicDermatitiss {
   atopicDermatitiss {
    id
    placeholder
    }
    ...
    if (existingData.atopicDermatitiss.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.atopicDermatitiss[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAtopicDermatitis.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchAtopicDermatitisData = async () => {
      const query = gql`
        query {
            atopicDermatitiss {
            id
      ...
      try {
        ...
        console.log(getData.atopicDermatitiss[0]);
        setQuestionsToDisplay(getData.atopicDermatitiss)
