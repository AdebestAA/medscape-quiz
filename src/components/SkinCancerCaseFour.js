...
const SkinCancerCaseFour = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteSkinCancerCaseFour($id: ID!) {
    deleteSkinCancerCaseFour(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createSkinCancerCaseFour($data: SkinCancerCaseFourCreateInput!) {
    createSkinCancerCaseFour(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishSkinCancerCaseFour($id: ID!) {
    publishSkinCancerCaseFour(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createSkinCancerCaseFour.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingSkinCancerCaseFours {
   skinCancerCaseFours {
    id
    placeholder
    }
    ...
    if (existingData.skinCancerCaseFours.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.skinCancerCaseFours[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishSkinCancerCaseFour.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchSkinCancerCaseFourData = async () => {
      const query = gql`
        query {
            skinCancerCaseFours {
            id
      ...
      try {
        ...
        console.log(getData.skinCancerCaseFours[0]);
        setQuestionsToDisplay(getData.skinCancerCaseFours)
...
export default SkinCancerCaseFour
