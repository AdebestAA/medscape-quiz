...
const SkinCancerCaseTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteSkinCancerCaseTwo($id: ID!) {
    deleteSkinCancerCaseTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createSkinCancerCaseTwo($data: SkinCancerCaseTwoCreateInput!) {
    createSkinCancerCaseTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishSkinCancerCaseTwo($id: ID!) {
    publishSkinCancerCaseTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createSkinCancerCaseTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingSkinCancerCaseTwos {
   skinCancerCaseTwos {
    id
    placeholder
    }
    ...
    if (existingData.skinCancerCaseTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.skinCancerCaseTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishSkinCancerCaseTwo.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchSkinCancerCaseTwoData = async () => {
      const query = gql`
        query {
            skinCancerCaseTwos {
            id
      ...
      try {
        ...
        console.log(getData.skinCancerCaseTwos[0]);
        setQuestionsToDisplay(getData.skinCancerCaseTwos)
...
export default SkinCancerCaseTwo
