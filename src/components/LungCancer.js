...
const LungCancer = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLungCancer($id: ID!) {
    deleteLungCancer(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLungCancer($data: LungCancerCreateInput!) {
    createLungCancer(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLungCancer($id: ID!) {
    publishLungCancer(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLungCancer.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   lungCancers {
    ...
    if (existingData.lungCancers.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.lungCancers[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLungCancer.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchLungCancerData = async () => {
      const query = gql`
        query {
            lungCancers {
            id
      ...
      try {
        ...
        console.log(getData.lungCancers[0]);
        setQuestionsToDisplay(getData.lungCancers)
