...
console.log(dataFromTypeForm);
 ...
 mutation deleteIgan($id: ID!) {
    deleteIgan(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createIgan($data: IganCreateInput!) {
    createIgan(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishIgan($id: ID!) {
    publishIgan(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createIgan.id;
...
const overwriteAndPublishQuest = async (newData) => {
  ...
  igans {
    ...
    if (existingData.igans.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.igans[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishIgan.id) {
      return true
    }
    else{
      return false
  ...
  useEffect(() => {
    ...
    const fetchIganData = async () => {
      ...
      try {
        ...
        console.log(getData.igans[0]);
        setQuestionsToDisplay(getData.igans)
