...
const CardiacSurgeryCaseOne = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCardiacSurgeryCaseOne($id: ID!) {
    deleteCardiacSurgeryCaseOne(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCardiacSurgeryCaseOne($data: CardiacSurgeryCaseOneCreateInput!) {
    createCardiacSurgeryCaseOne(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCardiacSurgeryCaseOne($id: ID!) {
    publishCardiacSurgeryCaseOne(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCardiacSurgeryCaseOne.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCardiacSurgeryCaseOnes {
  cardiacSurgeryCaseOnes {
    id
    placeholder
    }
    }
    ...
    if (existingData.cardiacSurgeryCaseOnes.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.cardiacSurgeryCaseOnes[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCardiacSurgeryCaseOne.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCardiacSurgeryCaseOneData = async () => {
      const query = gql`
        query {
            cardiacSurgeryCaseOnes {
            id
      ...
      try {
        ...
        console.log(getData.cardiacSurgeryCaseOnes[0]);
        setQuestionsToDisplay(getData.cardiacSurgeryCaseOnes)
   ...
   fetchCardiacSurgeryCaseOneData();
  }, [dataPosted]);
...
export default CardiacSurgeryCaseOne