...
const Transfusional = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteTransfusional($id: ID!) {
    deleteTransfusional(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createTransfusional($data: TransfusionalCreateInput!) {
    createTransfusional(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishTransfusional($id: ID!) {
    publishTransfusional(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createTransfusional.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingTransfusionals {
   transfusionals {
    id
    placeholder
    }
    ...
    if (existingData.transfusionals.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.transfusionals[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishTransfusional.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchTransfusionalData = async () => {
      const query = gql`
        query {
            transfusionals {
            id
      ...
      try {
        ...
        console.log(getData.transfusionals[0]);
        setQuestionsToDisplay(getData.transfusionals)
