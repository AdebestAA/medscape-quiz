...
const AhusCaseTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAhusCaseTwo($id: ID!) {
    deleteAhusCaseTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAhusCaseTwo($data: AhusCaseTwoCreateInput!) {
    createAhusCaseTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAhusCaseTwo($id: ID!) {
    publishAhusCaseTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAhusCaseTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   ahusCaseTwos {
    ...
    if (existingData.ahusCaseTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.ahusCaseTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAhusCaseTwo.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchAhusCaseTwoData = async () => {
      const query = gql`
        query {
            ahusCaseTwos {
            id
      ...
      try {
        ...
        console.log(getData.ahusCaseTwos[0]);
        setQuestionsToDisplay(getData.ahusCaseTwos)
