...
const AhusCaseThree = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAhusCaseThree($id: ID!) {
    deleteAhusCaseThree(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAhusCaseThree($data: AhusCaseThreeCreateInput!) {
    createAhusCaseThree(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAhusCaseThree($id: ID!) {
    publishAhusCaseThree(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAhusCaseThree.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingAhusCaseThrees {
   ahusCaseThrees {
    id
    placeholder
    }
    ...
    if (existingData.ahusCaseThrees.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.ahusCaseThrees[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAhusCaseThree.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchAhusCaseThreeData = async () => {
      const query = gql`
        query {
            ahusCaseThrees {
            id
      ...
      try {
        ...
        console.log(getData.ahusCaseThrees[0]);
        setQuestionsToDisplay(getData.ahusCaseThrees)
