...
const InfluenzaCaseTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteInfluenzaCaseTwo($id: ID!) {
    deleteInfluenzaCaseTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createInfluenzaCaseTwo($data: InfluenzaCaseTwoCreateInput!) {
    createInfluenzaCaseTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishInfluenzaCaseTwo($id: ID!) {
    publishInfluenzaCaseTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createInfluenzaCaseTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingInfluenzaCaseTwos {
  influenzaCaseTwos {
    id
    placeholder
    }
    ...
    if (existingData.influenzaCaseTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.influenzaCaseTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishInfluenzaCaseTwo.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchInfluenzaCaseTwoData = async () => {
      const query = gql`
        query {
            influenzaCaseTwos {
            id
      ...
      try {
        ...
        console.log(getData.influenzaCaseTwos[0]);
        setQuestionsToDisplay(getData.influenzaCaseTwos)
...
export default InfluenzaCaseTwo