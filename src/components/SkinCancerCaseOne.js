...
const SkinCancerCaseOne = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteSkinCancerCaseOne($id: ID!) {
    deleteSkinCancerCaseOne(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createSkinCancerCaseOne($data: SkinCancerCaseOneCreateInput!) {
    createSkinCancerCaseOne(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishSkinCancerCaseOne($id: ID!) {
    publishSkinCancerCaseOne(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createSkinCancerCaseOne.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingSkinCancerCaseOnes {
   skinCancerCaseOnes {
    id
    placeholder
    }
    ...
    if (existingData.skinCancerCaseOnes.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.skinCancerCaseOnes[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishSkinCancerCaseOne.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchSkinCancerCaseOneData = async () => {
      const query = gql`
        query {
            skinCancerCaseOnes {
            id
      ...
      try {
        ...
        console.log(getData.skinCancerCaseOnes[0]);
        setQuestionsToDisplay(getData.skinCancerCaseOnes)
...
export default SkinCancerCaseOne
