...
const SpinalMuscularAtrophyWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteSpinalMuscularAtrophyWebhook($id: ID!) {
    deleteSpinalMuscularAtrophyWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createSpinalMuscularAtrophyWebhook($data: SpinalMuscularAtrophyWebhookCreateInput!) {
    createSpinalMuscularAtrophyWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishSpinalMuscularAtrophyWebhook($id: ID!) {
    publishSpinalMuscularAtrophyWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createSpinalMuscularAtrophyWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingSpinalMuscularAtrophyWebhooks {
   spinalMuscularAtrophyWebhooks {
    ...
    if (existingData.spinalMuscularAtrophyWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.spinalMuscularAtrophyWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishSpinalMuscularAtrophyWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchSpinalMuscularAtrophyWebhookData = async () => {
      const query = gql`
        query {
            spinalMuscularAtrophyWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.spinalMuscularAtrophyWebhooks[0]);
        setQuestionsToDisplay(getData.spinalMuscularAtrophyWebhooks)
   ...
   fetchSpinalMuscularAtrophyWebhookData();
  }, [dataPosted]);
...
export default SpinalMuscularAtrophyWebhooks