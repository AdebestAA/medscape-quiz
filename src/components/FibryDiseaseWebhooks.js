...
const FibryDiseaseWebhook = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteFibryDiseaseWebhook($id: ID!) {
    deleteFibryDiseaseWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createFibryDiseaseWebhook($data: FibryDiseaseWebhookCreateInput!) {
    createFibryDiseaseWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishFibryDiseaseWebhook($id: ID!) {
    publishFibryDiseaseWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createFibryDiseaseWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingFibryDiseaseWebhooks {
   fibryDiseaseWebhooks {
    id
    placeholder
    }
    }
    ...
    if (existingData.fibryDiseaseWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.fibryDiseaseWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishFibryDiseaseWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchFibryDiseaseWebhookData = async () => {
      const query = gql`
        query {
            fibryDiseaseWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.fibryDiseaseWebhooks[0]);
        setQuestionsToDisplay(getData.fibryDiseaseWebhooks)
...
export default FibryDiseaseWebhook
