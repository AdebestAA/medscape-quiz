...
const CytomegalovirusCaseOne = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCytomegalovirusCaseOne($id: ID!) {
    deleteCytomegalovirusCaseOne(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCytomegalovirusCaseOne($data: CytomegalovirusCaseOneCreateInput!) {
    createCytomegalovirusCaseOne(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCytomegalovirusCaseOne($id: ID!) {
    publishCytomegalovirusCaseOne(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCytomegalovirusCaseOne.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCytomegalovirusCaseOnes {
  cytomegalovirusCaseOnes {
    id
    placeholder
    }
    }
    ...
    if (existingData.cytomegalovirusCaseOnes.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.cytomegalovirusCaseOnes[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCytomegalovirusCaseOne.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCytomegalovirusCaseOneData = async () => {
      const query = gql`
        query {
            cytomegalovirusCaseOnes {
            id
      ...
      try {
        ...
        console.log(getData.cytomegalovirusCaseOnes[0]);
        setQuestionsToDisplay(getData.cytomegalovirusCaseOnes)
   ...
   fetchCytomegalovirusCaseOneData();
  }, [dataPosted]);
...
export default CytomegalovirusCaseOne