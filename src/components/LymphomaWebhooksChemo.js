...
const LymphomaWebhooksChemo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLymphomaWebhooksChemo($id: ID!) {
    deleteLymphomaWebhooksChemo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLymphomaWebhooksChemo($data: LymphomaWebhooksChemoCreateInput!) {
    createLymphomaWebhooksChemo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLymphomaWebhooksChemo($id: ID!) {
    publishLymphomaWebhooksChemo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLymphomaWebhooksChemo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingLymphomaWebhooksChemos {
   lymphomaWebhooksChemos {
    id
    placeholder
    }
    }
    ...
    if (existingData.lymphomaWebhooksChemos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.lymphomaWebhooksChemos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLymphomaWebhooksChemo.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchLymphomaWebhooksChemoData = async () => {
      const query = gql`
        query {
            lymphomaWebhooksChemos {
            id
      ...
      try {
        ...
        console.log(getData.lymphomaWebhooksChemos[0]);
        setQuestionsToDisplay(getData.lymphomaWebhooksChemos)
   ...
   fetchLymphomaWebhooksChemoData();
  }, [dataPosted]);
...
export default LymphomaWebhooksChemo