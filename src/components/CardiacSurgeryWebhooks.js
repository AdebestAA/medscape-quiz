...
const CardiacSurgeryWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCardiacSurgeryWebhook($id: ID!) {
    deleteCardiacSurgeryWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCardiacSurgeryWebhook($data: CardiacSurgeryWebhookCreateInput!) {
    createCardiacSurgeryWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCardiacSurgeryWebhook($id: ID!) {
    publishCardiacSurgeryWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCardiacSurgeryWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCardiacSurgeryWebhooks {
   cardiacSurgeryWebhooks {
    id
    placeholder
    }
    }
    ...
    if (existingData.cardiacSurgeryWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.cardiacSurgeryWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCardiacSurgeryWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCardiacSurgeryWebhookData = async () => {
      const query = gql`
        query {
            cardiacSurgeryWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.cardiacSurgeryWebhooks[0]);
        setQuestionsToDisplay(getData.cardiacSurgeryWebhooks)
   ...
   fetchCardiacSurgeryWebhookData();
  }, [dataPosted]);
...
export default CardiacSurgeryWebhooks