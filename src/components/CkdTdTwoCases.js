...
const CkdTdTwoCases = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCkdTdTwoCase($id: ID!) {
    deleteCkdTdTwoCase(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCkdTdTwoCase($data: CkdTdTwoCaseCreateInput!) {
    createCkdTdTwoCase(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCkdTdTwoCase($id: ID!) {
    publishCkdTdTwoCase(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCkdTdTwoCase.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCkdTdTwoCases {
  ckdTdTwoCases {
    id
    placeholder
    }
    ...
    if (existingData.ckdTdTwoCases.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.ckdTdTwoCases[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCkdTdTwoCase.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCkdTdTwoCaseData = async () => {
      const query = gql`
        query {
           ckdTdTwoCases {
            id
      ...
      try {
        ...
        console.log(getData.ckdTdTwoCases[0]);
        setQuestionsToDisplay(getData.ckdTdTwoCases)
