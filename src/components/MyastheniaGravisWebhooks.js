...
const MyastheniaGravisWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteMyastheniaGravisWebhook($id: ID!) {
    deleteMyastheniaGravisWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createMyastheniaGravisWebhook($data: MyastheniaGravisWebhookCreateInput!) {
    createMyastheniaGravisWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishMyastheniaGravisWebhook($id: ID!) {
    publishMyastheniaGravisWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createMyastheniaGravisWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingMyastheniaGravisWebhooks {
   myastheniaGravisWebhooks {
    ...
    if (existingData.myastheniaGravisWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.myastheniaGravisWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishMyastheniaGravisWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchMyastheniaGravisWebhookData = async () => {
      const query = gql`
        query {
            myastheniaGravisWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.myastheniaGravisWebhooks[0]);
        setQuestionsToDisplay(getData.myastheniaGravisWebhooks)
   ...
   fetchMyastheniaGravisWebhookData();
  }, [dataPosted]);
...
export default MyastheniaGravisWebhooks