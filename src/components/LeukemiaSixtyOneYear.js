...
const LeukemiaSixtyOneYear = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLeukemiaSixtyOneYear($id: ID!) {
    deleteLeukemiaSixtyOneYear(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLeukemiaSixtyOneYear($data: LeukemiaSixtyOneYearCreateInput!) {
    createLeukemiaSixtyOneYear(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLeukemiaSixtyOneYear($id: ID!) {
    publishLeukemiaSixtyOneYear(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLeukemiaSixtyOneYear.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingLeukemiaSixtyOneYears {
   leukemiaSixtyOneYears {
    id
    placeholder
    }
    }
    ...
    if (existingData.leukemiaSixtyOneYears.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.leukemiaSixtyOneYears[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLeukemiaSixtyOneYear.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchLeukemiaSixtyOneYearData = async () => {
      const query = gql`
        query {
            leukemiaSixtyOneYears {
            id
      ...
      try {
        ...
        console.log(getData.leukemiaSixtyOneYears[0]);
        setQuestionsToDisplay(getData.leukemiaSixtyOneYears)
...
export default LeukemiaSixtyOneYear