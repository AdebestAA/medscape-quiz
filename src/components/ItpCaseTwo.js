...
const ItpCaseTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteItpCaseTwo($id: ID!) {
    deleteItpCaseTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createItpCaseTwo($data: ItpCaseTwoCreateInput!) {
    createItpCaseTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishItpCaseTwo($id: ID!) {
    publishItpCaseTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createItpCaseTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  ...
  itpCaseTwos {
    ...
    if (existingData.itpCaseTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.itpCaseTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishItpCaseTwo.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchItpCaseTwoData = async () => {
      const query = gql`
        query {
            itpCaseTwos {
            id
      ...
      try {
        ...
        console.log(getData.itpCaseTwos[0]);
        setQuestionsToDisplay(getData.itpCaseTwos)
