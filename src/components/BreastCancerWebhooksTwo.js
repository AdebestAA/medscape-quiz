...
const BreastCancerWebhooksTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteBreastCancerWebhooksTwo($id: ID!) {
    deleteBreastCancerWebhooksTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createBreastCancerWebhooksTwo($data: BreastCancerWebhooksTwoCreateInput!) {
    createBreastCancerWebhooksTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishBreastCancerWebhooksTwo($id: ID!) {
    publishBreastCancerWebhooksTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createBreastCancerWebhooksTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingBreastCancerWebhooksTwos {
   breastCancerWebhooksTwos {
    id
    placeholder
    }
    }
    ...
    if (existingData.breastCancerWebhooksTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.breastCancerWebhooksTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishBreastCancerWebhooksTwo.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchBreastCancerWebhooksTwoData = async () => {
      const query = gql`
        query {
            breastCancerWebhooksTwos {
            id
      ...
      try {
        ...
        console.log(getData.breastCancerWebhooksTwos[0]);
        setQuestionsToDisplay(getData.breastCancerWebhooksTwos)
   ...
   fetchBreastCancerWebhooksTwoData();
  }, [dataPosted]);
...
export default BreastCancerWebhooksTwo