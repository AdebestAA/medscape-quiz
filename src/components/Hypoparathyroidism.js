...
const Hypoparathyroidism = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteHypoparathyroidism($id: ID!) {
    deleteHypoparathyroidism(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHypoparathyroidism($data: HypoparathyroidismCreateInput!) {
    createHypoparathyroidism(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishHypoparathyroidism($id: ID!) {
    publishHypoparathyroidism(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHypoparathyroidism.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingHypoparathyroidisms {
   hypoparathyroidisms {
    id
    placeholder
    }
    ...
    if (existingData.hypoparathyroidisms.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.hypoparathyroidisms[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHypoparathyroidism.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchHypoparathyroidismData = async () => {
      const query = gql`
        query {
            hypoparathyroidisms {
            id
      ...
      try {
        ...
        console.log(getData.hypoparathyroidisms[0]);
        setQuestionsToDisplay(getData.hypoparathyroidisms)
