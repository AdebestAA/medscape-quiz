...
overwriteAndPublishQuest(dataFromTypeForm);
 ...
 mutation deleteNCkd($id: ID!) {
    deleteNCkd(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createNCkd($data: NCkdCreateInput!) {
    createNCkd(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishNCkd($id: ID!) {
    publishNCkd(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createNCkd.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   nCkds {
    ...
    if (existingData.nCkds.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.nCkds[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishNCkd.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchNCkdData = async () => {
      ...
      try {
        ...
        console.log(getData.nCkds[0]);
        setQuestionsToDisplay(getData.nCkds)
