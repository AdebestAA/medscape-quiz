...
const PsoriaticDiseasesCaseOne = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deletePsoriaticDiseasesCaseOne($id: ID!) {
    deletePsoriaticDiseasesCaseOne(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createPsoriaticDiseasesCaseOne($data: PsoriaticDiseasesCaseOneCreateInput!) {
    createPsoriaticDiseasesCaseOne(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishPsoriaticDiseasesCaseOne($id: ID!) {
    publishPsoriaticDiseasesCaseOne(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createPsoriaticDiseasesCaseOne.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingPsoriaticDiseasesCaseOnes {
  psoriaticDiseasesCaseOnes {
    id
    placeholder
    }
    }
    ...
    if (existingData.psoriaticDiseasesCaseOnes.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.psoriaticDiseasesCaseOnes[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishPsoriaticDiseasesCaseOne.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchPsoriaticDiseasesCaseOneData = async () => {
      const query = gql`
        query {
            psoriaticDiseasesCaseOnes {
            id
      ...
      try {
        ...
        console.log(getData.psoriaticDiseasesCaseOnes[0]);
        setQuestionsToDisplay(getData.psoriaticDiseasesCaseOnes)
   ...
   fetchPsoriaticDiseasesCaseOneData();
  }, [dataPosted]);
...
export default PsoriaticDiseasesCaseOne