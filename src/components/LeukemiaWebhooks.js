...
const LeukemiaWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLeukemiaWebhook($id: ID!) {
    deleteLeukemiaWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLeukemiaWebhook($data: LeukemiaWebhookCreateInput!) {
    createLeukemiaWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLeukemiaWebhook($id: ID!) {
    publishLeukemiaWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLeukemiaWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingLeukemiaWebhooks {
   leukemiaWebhooks {
    id
    placeholder
    }
    ...
    if (existingData.leukemiaWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.leukemiaWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLeukemiaWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchLeukemiaWebhookData = async () => {
      const query = gql`
        query {
            leukemiaWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.leukemiaWebhooks[0]);
        setQuestionsToDisplay(getData.leukemiaWebhooks)
...
export default LeukemiaWebhooks
