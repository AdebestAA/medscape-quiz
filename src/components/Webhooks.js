...
const Webhooks = ({dataFromTypeForm}) => {
...
const publishQuestMutation = gql`
  mutation publishWebhook($id: ID!) {
    publishWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   webhooks {
    ...
    if (existingData.webhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.webhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishWebhook.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchWebhookData = async () => {
      ...
      try {
        ...
        console.log(getData.webhooks[0]);
        setQuestionsToDisplay(getData.webhooks)
