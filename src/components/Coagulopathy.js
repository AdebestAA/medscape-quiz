...
const Coagulopathy = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCoagulopathy($id: ID!) {
    deleteCoagulopathy(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCoagulopathy($data: CoagulopathyCreateInput!) {
    createCoagulopathy(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCoagulopathy($id: ID!) {
    publishCoagulopathy(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCoagulopathy.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCoagulopathies {
   coagulopathies {
    id
    placeholder
    }
    ...
    if (existingData.coagulopathies.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.coagulopathies[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCoagulopathy.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCoagulopathyData = async () => {
      const query = gql`
        query {
            coagulopathies {
            id
      ...
      try {
        ...
        console.log(getData.coagulopathies[0]);
        setQuestionsToDisplay(getData.coagulopathies)
