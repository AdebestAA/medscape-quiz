...
const ProstateCancer = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteProstateCancer($id: ID!) {
    deleteProstateCancer(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createProstateCancer($data: ProstateCancerCreateInput!) {
    createProstateCancer(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishProstateCancer($id: ID!) {
    publishProstateCancer(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createProstateCancer.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingProstateCancers {
   prostateCancers {
    id
    placeholder
    }
    ...
    if (existingData.prostateCancers.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.prostateCancers[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishProstateCancer.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchProstateCancerData = async () => {
      const query = gql`
        query {
            prostateCancers {
            id
      ...
      try {
        ...
        console.log(getData.prostateCancers[0]);
        setQuestionsToDisplay(getData.prostateCancers)
