...
const HypoalbuminemiaTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteHypoalbuminemiaTwo($id: ID!) {
    deleteHypoalbuminemiaTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHypoalbuminemiaTwo($data: HypoalbuminemiaTwoCreateInput!) {
    createHypoalbuminemiaTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishHypoalbuminemiaTwo($id: ID!) {
    publishHypoalbuminemiaTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHypoalbuminemiaTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingHypoalbuminemiaTwos {
   hypoalbuminemiaTwos {
    id
    placeholder
    }
    ...
    if (existingData.hypoalbuminemiaTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.hypoalbuminemiaTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHypoalbuminemiaTwo.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchHypoalbuminemiaTwoData = async () => {
      const query = gql`
        query {
            hypoalbuminemiaTwos {
            id
      ...
      try {
        ...
        console.log(getData.hypoalbuminemiaTwos[0]);
        setQuestionsToDisplay(getData.hypoalbuminemiaTwos)
...
export default HypoalbuminemiaTwo
