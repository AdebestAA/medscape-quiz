...
const LymphomaSixtySixYear = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLymphomaSixtySixYear($id: ID!) {
    deleteLymphomaSixtySixYear(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLymphomaSixtySixYear($data: LymphomaSixtySixYearCreateInput!) {
    createLymphomaSixtySixYear(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLymphomaSixtySixYear($id: ID!) {
    publishLymphomaSixtySixYear(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLymphomaSixtySixYear.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingLymphomaSixtySixYears {
   lymphomaSixtySixYears {
    id
    placeholder
    }
    }
    ...
    if (existingData.lymphomaSixtySixYears.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.lymphomaSixtySixYears[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLymphomaSixtySixYear.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchLymphomaSixtySixYearData = async () => {
      const query = gql`
        query {
           lymphomaSixtySixYears {
            id
      ...
      try {
        ...
        console.log(getData.lymphomaSixtySixYears[0]);
        setQuestionsToDisplay(getData.lymphomaSixtySixYears)
...
export default LymphomaSixtySixYear