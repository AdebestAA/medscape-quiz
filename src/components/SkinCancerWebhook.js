...
const SkinCancerWebhook = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteSkinCancerWebhook($id: ID!) {
    deleteSkinCancerWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createSkinCancerWebhook($data: SkinCancerWebhookCreateInput!) {
    createSkinCancerWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishSkinCancerWebhook($id: ID!) {
    publishSkinCancerWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createSkinCancerWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingSkinCancerWebhooks {
   skinCancerWebhooks {
    id
    placeholder
    }
    ...
    if (existingData.skinCancerWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.skinCancerWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishSkinCancerWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchSkinCancerWebhookData = async () => {
      const query = gql`
        query {
            skinCancerWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.skinCancerWebhooks[0]);
        setQuestionsToDisplay(getData.skinCancerWebhooks)
...
export default SkinCancerWebhook
