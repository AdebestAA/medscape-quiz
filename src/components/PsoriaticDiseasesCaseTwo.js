...
const PsoriaticDiseasesCaseTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deletePsoriaticDiseasesCaseTwo($id: ID!) {
    deletePsoriaticDiseasesCaseTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createPsoriaticDiseasesCaseTwo($data: PsoriaticDiseasesCaseTwoCreateInput!) {
    createPsoriaticDiseasesCaseTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishPsoriaticDiseasesCaseTwo($id: ID!) {
    publishPsoriaticDiseasesCaseTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createPsoriaticDiseasesCaseTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingPsoriaticDiseasesCaseTwos {
  psoriaticDiseasesCaseTwos {
    id
    placeholder
    }
    }
    ...
    if (existingData.psoriaticDiseasesCaseTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.psoriaticDiseasesCaseTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishPsoriaticDiseasesCaseTwo.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchPsoriaticDiseasesCaseTwoData = async () => {
      const query = gql`
        query {
            psoriaticDiseasesCaseTwos {
            id
      ...
      try {
        ...
        console.log(getData.psoriaticDiseasesCaseTwos[0]);
        setQuestionsToDisplay(getData.psoriaticDiseasesCaseTwos)
   ...
   fetchPsoriaticDiseasesCaseTwoData();
  }, [dataPosted]);
...
export default PsoriaticDiseasesCaseTwo