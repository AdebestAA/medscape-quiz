...
const PolycythemiaVeraMpns = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deletePolycythemiaVeraMpn($id: ID!) {
    deletePolycythemiaVeraMpn(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createPolycythemiaVeraMpn($data: PolycythemiaVeraMpnCreateInput!) {
    createPolycythemiaVeraMpn(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishPolycythemiaVeraMpn($id: ID!) {
    publishPolycythemiaVeraMpn(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createPolycythemiaVeraMpn.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingPolycythemiaVeraMpns {
   polycythemiaVeraMpns {
    id
    placeholder
    }
    }
    ...
    if (existingData.polycythemiaVeraMpns.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.polycythemiaVeraMpns[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishPolycythemiaVeraMpn.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchPolycythemiaVeraMpnData = async () => {
      const query = gql`
        query {
            polycythemiaVeraMpns {
            id
      ...
      try {
        ...
        console.log(getData.polycythemiaVeraMpns[0]);
        setQuestionsToDisplay(getData.polycythemiaVeraMpns)
...
export default PolycythemiaVeraMpns