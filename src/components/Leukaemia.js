...
const Leukaemia = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLeukaemia($id: ID!) {
    deleteLeukaemia(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLeukaemia($data: LeukaemiaCreateInput!) {
    createLeukaemia(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLeukaemia($id: ID!) {
    publishLeukaemia(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLeukaemia.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   leukaemias {
    ...
    if (existingData.leukaemias.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.leukaemias[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLeukaemia.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchLeukaemiaData = async () => {
      const query = gql`
        query {
            leukaemias {
            id
      ...
      try {
        ...
        console.log(getData.leukaemias[0]);
        setQuestionsToDisplay(getData.leukaemias)
