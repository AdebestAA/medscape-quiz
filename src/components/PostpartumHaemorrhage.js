...
const PostpartumHaemorrhage = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deletePostpartumHaemorrhage($id: ID!) {
    deletePostpartumHaemorrhage(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createPostpartumHaemorrhage($data: PostpartumHaemorrhageCreateInput!) {
    createPostpartumHaemorrhage(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishPostpartumHaemorrhage($id: ID!) {
    publishPostpartumHaemorrhage(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createPostpartumHaemorrhage.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingPostpartumHaemorrhages {
   postpartumHaemorrhages {
    id
    placeholder
    }
    }
    ...
    if (existingData.postpartumHaemorrhages.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.postpartumHaemorrhages[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishPostpartumHaemorrhage.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchPostpartumHaemorrhageData = async () => {
      const query = gql`
        query {
            postpartumHaemorrhages {
            id
      ...
      try {
        ...
        console.log(getData.postpartumHaemorrhages[0]);
        setQuestionsToDisplay(getData.postpartumHaemorrhages)
   ...
   fetchPostpartumHaemorrhageData();
  }, [dataPosted]);
...
export default PostpartumHaemorrhage