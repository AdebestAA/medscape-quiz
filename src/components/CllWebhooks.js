...
const CllWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCllWebhook($id: ID!) {
    deleteCllWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCllWebhook($data: CllWebhookCreateInput!) {
    createCllWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCllWebhook($id: ID!) {
    publishCllWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCllWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   cllWebhooks {
    ...
    if (existingData.cllWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.cllWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCllWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCllWebhookData = async () => {
      const query = gql`
        query {
            cllWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.cllWebhooks[0]);
        setQuestionsToDisplay(getData.cllWebhooks)
