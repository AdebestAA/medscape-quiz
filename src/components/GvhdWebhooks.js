...
const GvhdWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteGvhdWebhook($id: ID!) {
    deleteGvhdWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createGvhdWebhook($data: GvhdWebhookCreateInput!) {
    createGvhdWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishGvhdWebhook($id: ID!) {
    publishGvhdWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createGvhdWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   gvhdWebhooks {
    ...
    if (existingData.gvhdWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.gvhdWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishGvhdWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchGvhdWebhookData = async () => {
      const query = gql`
        query {
            gvhdWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.gvhdWebhooks[0]);
        setQuestionsToDisplay(getData.gvhdWebhooks)
