...
console.log(dataFromTypeForm);
 ...
 mutation deleteHiv($id: ID!) {
    deleteHiv(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHiv($data: HivCreateInput!) {
    createHiv(data: $data) {
      id
      placeholder
    }
...
const publishQuestMutation = gql`
  mutation publishHiv($id: ID!) {
    publishHiv(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHiv.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   hivs {
    ...
    if (existingData.hivs.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.hivs[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHiv.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchHivData = async () => {
      ...
      try {
        ...
        console.log(getData.hivs[0]);
        setQuestionsToDisplay(getData.hivs)
...
{quest?.properties?.choices?.map((choice,choiceIndex)=>{
    ...
    return (
        ...
        <aside key={choice.id} className={`my-2 ${quest?.properties?.choices?.length  > 50 ? " w-[30%] mx-[2px] " : " w-[100%]"}  rounded-sm text-start text-[0.95rem] border-[2px] border-[navy] flex items-start px-2 py-2 gap-x-2 bg-[#D7DDE8] hover:bg-[#A6BCDA]  cursor-pointer`}
        onClick={()=>{
            if (step + 1 == questionsToDisplay[0]?.placeholder?.fields?.length) {
                return
            }
