...
const Amyloidosis = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAmyloidosis($id: ID!) {
    deleteAmyloidosis(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAmyloidosis($data: AmyloidosisCreateInput!) {
    createAmyloidosis(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAmyloidosis($id: ID!) {
    publishAmyloidosis(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAmyloidosis.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   amyloidoses {
    ...
    if (existingData.amyloidoses.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.amyloidoses[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAmyloidosis.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchAmyloidosisData = async () => {
      const query = gql`
        query {
            amyloidoses {
            id
      ...
      try {
        ...
        console.log(getData.amyloidoses[0]);
        setQuestionsToDisplay(getData.amyloidoses)
