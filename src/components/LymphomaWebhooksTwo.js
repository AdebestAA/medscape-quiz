...
const LymphomaWebhooksTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLymphomaWebhooksTwo($id: ID!) {
    deleteLymphomaWebhooksTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLymphomaWebhooksTwo($data: LymphomaWebhooksTwoCreateInput!) {
    createLymphomaWebhooksTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLymphomaWebhooksTwo($id: ID!) {
    publishLymphomaWebhooksTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLymphomaWebhooksTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingLymphomaWebhooksTwos {
   lymphomaWebhooksTwos {
    id
    placeholder
    }
    }
    ...
    if (existingData.lymphomaWebhooksTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.lymphomaWebhooksTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLymphomaWebhooksTwo.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchLymphomaWebhooksTwoData = async () => {
      const query = gql`
        query {
            lymphomaWebhooksTwos {
            id
      ...
      try {
        ...
        console.log(getData.lymphomaWebhooksTwos[0]);
        setQuestionsToDisplay(getData.lymphomaWebhooksTwos)
...
export default LymphomaWebhooksTwo