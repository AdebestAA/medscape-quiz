...
const Hepatitis = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteHepatitis($id: ID!) {
    deleteHepatitis(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHepatitis($data: HepatitisCreateInput!) {
    createHepatitis(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishHepatitis($id: ID!) {
    publishHepatitis(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHepatitis.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   hepatitiss {
    ...
    if (existingData.hepatitiss.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.hepatitiss[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHepatitis.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchHepatitisData = async () => {
      const query = gql`
        query {
            hepatitiss {
            id
      ...
      try {
        ...
        console.log(getData.hepatitiss[0]);
        setQuestionsToDisplay(getData.hepatitiss)
