...
const LymphomaGeoblock = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLymphomaGeoblock($id: ID!) {
    deleteLymphomaGeoblock(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLymphomaGeoblock($data: LymphomaGeoblockCreateInput!) {
    createLymphomaGeoblock(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLymphomaGeoblock($id: ID!) {
    publishLymphomaGeoblock(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLymphomaGeoblock.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingLymphomaGeoblocks {
   lymphomaGeoblocks {
    id
    placeholder
    }
    ...
    if (existingData.lymphomaGeoblocks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.lymphomaGeoblocks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLymphomaGeoblock.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchLymphomaGeoblockData = async () => {
      const query = gql`
        query {
            lymphomaGeoblocks {
            id
      ...
      try {
        ...
        console.log(getData.lymphomaGeoblocks[0]);
        setQuestionsToDisplay(getData.lymphomaGeoblocks)
...
export default LymphomaGeoblock
