...
const LymphomaWebhook = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLymphomaWebhook($id: ID!) {
    deleteLymphomaWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLymphomaWebhook($data: LymphomaWebhookCreateInput!) {
    createLymphomaWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLymphomaWebhook($id: ID!) {
    publishLymphomaWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLymphomaWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingLymphomaWebhooks {
   lymphomaWebhooks {
    id
    placeholder
    }
    ...
    if (existingData.lymphomaWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.lymphomaWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLymphomaWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchLymphomaWebhookData = async () => {
      const query = gql`
        query {
            lymphomaWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.lymphomaWebhooks[0]);
        setQuestionsToDisplay(getData.lymphomaWebhooks)
...
export default LymphomaWebhook