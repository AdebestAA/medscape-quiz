...
const Osteoporosis = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteOsteoporosis($id: ID!) {
    deleteOsteoporosis(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createOsteoporosis($data: OsteoporosisCreateInput!) {
    createOsteoporosis(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishOsteoporosis($id: ID!) {
    publishOsteoporosis(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createOsteoporosis.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   osteoporoses {
    ...
    if (existingData.osteoporoses.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.osteoporoses[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishOsteoporosis.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchOsteoporosisData = async () => {
      const query = gql`
        query {
            osteoporoses {
            id
      ...
      try {
        ...
        console.log(getData.osteoporoses[0]);
        setQuestionsToDisplay(getData.osteoporoses)
