...
const LeukemiaNineYear = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLeukemiaNineYear($id: ID!) {
    deleteLeukemiaNineYear(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLeukemiaNineYear($data: LeukemiaNineYearCreateInput!) {
    createLeukemiaNineYear(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLeukemiaNineYear($id: ID!) {
    publishLeukemiaNineYear(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLeukemiaNineYear.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingLeukemiaNineYears {
   leukemiaNineYears {
    id
    placeholder
    }
    ...
    if (existingData.leukemiaNineYears.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.leukemiaNineYears[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLeukemiaNineYear.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchLeukemiaNineYearData = async () => {
      const query = gql`
        query {
            leukemiaNineYears {
            id
      ...
      try {
        ...
        console.log(getData.leukemiaNineYears[0]);
        setQuestionsToDisplay(getData.leukemiaNineYears)
...
export default LeukemiaNineYear