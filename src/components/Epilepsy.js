...
const Epilepsy = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteEpilepsy($id: ID!) {
    deleteEpilepsy(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createEpilepsy($data: EpilepsyCreateInput!) {
    createEpilepsy(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishEpilepsy($id: ID!) {
    publishEpilepsy(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createEpilepsy.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   epilepsies {
    ...
    if (existingData.epilepsies.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.epilepsies[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishEpilepsy.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchEpilepsyData = async () => {
      ...
      try {
        ...
        console.log(getData.epilepsies[0]);
        setQuestionsToDisplay(getData.epilepsies)
