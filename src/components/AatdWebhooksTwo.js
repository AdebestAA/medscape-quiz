...
const AatdWebhooksTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAatdWebhooksTwo($id: ID!) {
    deleteAatdWebhooksTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAatdWebhooksTwo($data: AatdWebhooksTwoCreateInput!) {
    createAatdWebhooksTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAatdWebhooksTwo($id: ID!) {
    publishAatdWebhooksTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAatdWebhooksTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingAatdWebhooksTwos {
   aatdWebhooksTwos {
    id
    placeholder
    }
    ...
    if (existingData.aatdWebhooksTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.aatdWebhooksTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAatdWebhooksTwo.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchAatdWebhooksTwoData = async () => {
      const query = gql`
        query {
            aatdWebhooksTwos {
            id
      ...
      try {
        ...
        console.log(getData.aatdWebhooksTwos[0]);
        setQuestionsToDisplay(getData.aatdWebhooksTwos)
...
export default AatdWebhooksTwo