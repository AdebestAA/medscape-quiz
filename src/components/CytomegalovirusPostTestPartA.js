...
const CytomegalovirusPostTestPartA = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCytomegalovirusPostTestPartA($id: ID!) {
    deleteCytomegalovirusPostTestPartA(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCytomegalovirusPostTestPartA($data: CytomegalovirusPostTestPartACreateInput!) {
    createCytomegalovirusPostTestPartA(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCytomegalovirusPostTestPartA($id: ID!) {
    publishCytomegalovirusPostTestPartA(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCytomegalovirusPostTestPartA.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCytomegalovirusPostTestPartAs {
  cytomegalovirusPostTestPartAs {
    id
    placeholder
    }
    }
    ...
    if (existingData.cytomegalovirusPostTestPartAs.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.cytomegalovirusPostTestPartAs[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCytomegalovirusPostTestPartA.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCytomegalovirusPostTestPartAData = async () => {
      const query = gql`
        query {
            cytomegalovirusPostTestPartAs {
            id
      ...
      try {
        ...
        console.log(getData.cytomegalovirusPostTestPartAs[0]);
        setQuestionsToDisplay(getData.cytomegalovirusPostTestPartAs)
   ...
   fetchCytomegalovirusPostTestPartAData();
  }, [dataPosted]);
...
export default CytomegalovirusPostTestPartA