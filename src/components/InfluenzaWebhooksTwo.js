...
const InfluenzaWebhooksTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteInfluenzaWebhooksTwo($id: ID!) {
    deleteInfluenzaWebhooksTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createInfluenzaWebhooksTwo($data: InfluenzaWebhooksTwoCreateInput!) {
    createInfluenzaWebhooksTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishInfluenzaWebhooksTwo($id: ID!) {
    publishInfluenzaWebhooksTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createInfluenzaWebhooksTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingInfluenzaWebhooksTwos {
   influenzaWebhooksTwos {
    id
    placeholder
    }
    }
    ...
    if (existingData.influenzaWebhooksTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.influenzaWebhooksTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishInfluenzaWebhooksTwo.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchInfluenzaWebhooksTwoData = async () => {
      const query = gql`
        query {
            influenzaWebhooksTwos {
            id
      ...
      try {
        ...
        console.log(getData.influenzaWebhooksTwos[0]);
        setQuestionsToDisplay(getData.influenzaWebhooksTwos)
...
export default InfluenzaWebhooksTwo