...
const PrimaryMyelofibrosis = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deletePrimaryMyelofibrosi($id: ID!) {
    deletePrimaryMyelofibrosi(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createPrimaryMyelofibrosi($data: PrimaryMyelofibrosiCreateInput!) {
    createPrimaryMyelofibrosi(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishPrimaryMyelofibrosi($id: ID!) {
    publishPrimaryMyelofibrosi(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createPrimaryMyelofibrosi.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingPrimaryMyelofibrosis {
   primaryMyelofibrosis {
    ...
    if (existingData.primaryMyelofibrosis.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.primaryMyelofibrosis[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishPrimaryMyelofibrosi.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchPrimaryMyelofibrosiData = async () => {
      const query = gql`
        query {
            primaryMyelofibrosis {
            id
      ...
      try {
        ...
        console.log(getData.primaryMyelofibrosis[0]);
        setQuestionsToDisplay(getData.primaryMyelofibrosis)
...
export default PrimaryMyelofibrosis
