...
const ItpCaseOne = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteItpCaseOne($id: ID!) {
    deleteItpCaseOne(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createItpCaseOne($data: ItpCaseOneCreateInput!) {
    createItpCaseOne(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishItpCaseOne($id: ID!) {
    publishItpCaseOne(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createItpCaseOne.id;
...
const overwriteAndPublishQuest = async (newData) => {
  ...
  itpCaseOnes {
    ...
    if (existingData.itpCaseOnes.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.itpCaseOnes[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishItpCaseOne.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchItpCaseOneData = async () => {
      const query = gql`
        query {
            itpCaseOnes {
            id
      ...
      try {
        ...
        console.log(getData.itpCaseOnes[0]);
        setQuestionsToDisplay(getData.itpCaseOnes)
