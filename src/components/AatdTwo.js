...
const AatdTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAatdTwo($id: ID!) {
    deleteAatdTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAatdTwo($data: AatdTwoCreateInput!) {
    createAatdTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAatdTwo($id: ID!) {
    publishAatdTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAatdTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   aatdTwos {
    ...
    if (existingData.aatdTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.aatdTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAatdTwo.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchAatdTwoData = async () => {
      ...
      try {
        ...
        console.log(getData.aatdTwos[0]);
        setQuestionsToDisplay(getData.aatdTwos)
