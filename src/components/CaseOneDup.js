...
const CaseOneDup = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCaseOneDup($id: ID!) {
    deleteCaseOneDup(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCaseOneDup($data: CaseOneDupCreateInput!) {
    createCaseOneDup(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCaseOneDup($id: ID!) {
    publishCaseOneDup(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCaseOneDup.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   caseOneDups {
    ...
    if (existingData.caseOneDups.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.caseOneDups[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCaseOneDup.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCaseOneDupData = async () => {
      const query = gql`
        query {
            caseOneDups {
            id
      ...
      try {
        ...
        console.log(getData.caseOneDups[0]);
        setQuestionsToDisplay(getData.caseOneDups)
