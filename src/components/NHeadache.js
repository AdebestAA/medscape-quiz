...
const NHeadache = ({dataFromTypeForm}) => {
...
overwriteAndPublishQuest(dataFromTypeForm);
 ...
 mutation deleteNHeadache($id: ID!) {
    deleteNHeadache(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createNHeadache($data: NHeadacheCreateInput!) {
    createNHeadache(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishNHeadache($id: ID!) {
    publishNHeadache(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createNHeadache.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   nHeadaches {
    ...
    if (existingData.nHeadaches.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.nHeadaches[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishNHeadache.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchNHeadacheData = async () => {
      const query = gql`
        query {
            nHeadaches {
            id
      ...
      try {
        ...
        console.log(getData.nHeadaches[0]);
        setQuestionsToDisplay(getData.nHeadaches)
