...
const CrohnDiseases = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCrohnDiseases($id: ID!) {
    deleteCrohnDiseases(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCrohnDiseases($data: CrohnDiseasesCreateInput!) {
    createCrohnDiseases(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCrohnDiseases($id: ID!) {
    publishCrohnDiseases(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCrohnDiseases.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCrohnDiseasess {
   crohnDiseasess {
    id
    placeholder
    }
    ...
    if (existingData.crohnDiseasess.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.crohnDiseasess[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCrohnDiseases.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCrohnDiseasesData = async () => {
      const query = gql`
        query {
            crohnDiseasess {
            id
      ...
      try {
        ...
        console.log(getData.crohnDiseasess[0]);
        setQuestionsToDisplay(getData.crohnDiseasess)
