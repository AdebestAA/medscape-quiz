...
const AhusCaseOne = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAhusCaseOne($id: ID!) {
    deleteAhusCaseOne(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAhusCaseOne($data: AhusCaseOneCreateInput!) {
    createAhusCaseOne(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAhusCaseOne($id: ID!) {
    publishAhusCaseOne(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAhusCaseOne.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   ahusCaseOnes {
    ...
    if (existingData.ahusCaseOnes.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.ahusCaseOnes[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAhusCaseOne.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchAhusCaseOneData = async () => {
      const query = gql`
        query {
            ahusCaseOnes {
            id
      ...
      try {
        ...
        console.log(getData.ahusCaseOnes[0]);
        setQuestionsToDisplay(getData.ahusCaseOnes)
