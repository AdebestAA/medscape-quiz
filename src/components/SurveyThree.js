...
const SurveyThree = ({dataFromTypeForm}) => {
...
SurveyThree
 ...
 mutation deleteSurveyThree($id: ID!) {
    deleteSurveyThree(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createSurveyThree($data: SurveyThreeCreateInput!) {
    createSurveyThree(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishSurveyThree($id: ID!) {
    publishSurveyThree(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   surveyThrees {
    ...
    if (existingData.surveyThrees.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.surveyThrees[0].id;
  ...
  placeholder: newData
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createSurveyThree.id;
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishSurveyThree.id) {
      return true
    }
    else{
      false
 ...
 useEffect(() => {
    ...
    const fetchSurveyThreeData = async () => {
      const query = gql`
        query {
            surveyThrees {
            id
      ...
      try {
        ...
        console.log(getData.surveyThrees[0]);
        setQuestionsToDisplay(getData.surveyThrees)
