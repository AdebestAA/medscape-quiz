...
const LeukemiaCaseTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLeukemiaCaseTwo($id: ID!) {
    deleteLeukemiaCaseTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLeukemiaCaseTwo($data: LeukemiaCaseTwoCreateInput!) {
    createLeukemiaCaseTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLeukemiaCaseTwo($id: ID!) {
    publishLeukemiaCaseTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLeukemiaCaseTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingLeukemiaCaseTwos {
  leukemiaCaseTwos {
    id
    placeholder
    }
    ...
    if (existingData.leukemiaCaseTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.leukemiaCaseTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLeukemiaCaseTwo.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchLeukemiaCaseTwoData = async () => {
      const query = gql`
        query {
            leukemiaCaseTwos {
            id
      ...
      try {
        ...
        console.log(getData.leukemiaCaseTwos[0]);
        setQuestionsToDisplay(getData.leukemiaCaseTwos)
...
export default LeukemiaCaseTwo