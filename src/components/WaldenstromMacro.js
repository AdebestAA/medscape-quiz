...
const WaldenstromMacro = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteWaldenstromMacro($id: ID!) {
    deleteWaldenstromMacro(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createWaldenstromMacro($data: WaldenstromMacroCreateInput!) {
    createWaldenstromMacro(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishWaldenstromMacro($id: ID!) {
    publishWaldenstromMacro(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createWaldenstromMacro.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingWaldenstromMacros {
   waldenstromMacros {
    id
    placeholder
    }
    ...
    if (existingData.waldenstromMacros.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.waldenstromMacros[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishWaldenstromMacro.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchWaldenstromMacroData = async () => {
      const query = gql`
        query {
            waldenstromMacros {
            id
      ...
      try {
        ...
        console.log(getData.waldenstromMacros[0]);
        setQuestionsToDisplay(getData.waldenstromMacros)
...
export default WaldenstromMacro
