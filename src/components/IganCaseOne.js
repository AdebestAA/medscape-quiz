...
const IganCaseOne = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteIganCaseOne($id: ID!) {
    deleteIganCaseOne(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createIganCaseOne($data: IganCaseOneCreateInput!) {
    createIganCaseOne(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishIganCaseOne($id: ID!) {
    publishIganCaseOne(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createIganCaseOne.id;
...
const overwriteAndPublishQuest = async (newData) => {
  ...
  iganCaseOnes {
    ...
    if (existingData.iganCaseOnes.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.iganCaseOnes[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishIganCaseOne.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchIganCaseOneData = async () => {
      const query = gql`
        query {
            iganCaseOnes {
            id
      ...
      try {
        ...
        console.log(getData.iganCaseOnes[0]);
        setQuestionsToDisplay(getData.iganCaseOnes)
