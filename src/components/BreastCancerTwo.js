...
const BreastCancerTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteBreastCancerTwo($id: ID!) {
    deleteBreastCancerTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createBreastCancerTwo($data: BreastCancerTwoCreateInput!) {
    createBreastCancerTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishBreastCancerTwo($id: ID!) {
    publishBreastCancerTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createBreastCancerTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingBreastCancerTwos {
   breastCancerTwos {
    id
    placeholder
    }
    ...
    if (existingData.breastCancerTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.breastCancerTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishBreastCancerTwo.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchBreastCancerTwoData = async () => {
      const query = gql`
        query {
            breastCancerTwos {
            id
      ...
      try {
        ...
        console.log(getData.breastCancerTwos[0]);
        setQuestionsToDisplay(getData.breastCancerTwos)
...
export default BreastCancerTwo