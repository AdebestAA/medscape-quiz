...
const BreastCancer = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteBreastCancer($id: ID!) {
    deleteBreastCancer(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createBreastCancer($data: BreastCancerCreateInput!) {
    createBreastCancer(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishBreastCancer($id: ID!) {
    publishBreastCancer(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createBreastCancer.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingBreastCancers {
   breastCancers {
    id
    placeholder
    }
    ...
    if (existingData.breastCancers.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.breastCancers[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishBreastCancer.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchBreastCancerData = async () => {
      ...
      try {
        ...
        console.log(getData.breastCancers[0]);
        setQuestionsToDisplay(getData.breastCancers)
