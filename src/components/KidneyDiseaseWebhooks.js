...
const KidneyDiseaseWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteKidneyDiseaseWebhook($id: ID!) {
    deleteKidneyDiseaseWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createKidneyDiseaseWebhook($data: KidneyDiseaseWebhookCreateInput!) {
    createKidneyDiseaseWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishKidneyDiseaseWebhook($id: ID!) {
    publishKidneyDiseaseWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createKidneyDiseaseWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingKidneyDiseaseWebhooks {
   kidneyDiseaseWebhooks {
    id
    placeholder
    }
    }
    ...
    if (existingData.kidneyDiseaseWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.kidneyDiseaseWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishKidneyDiseaseWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchKidneyDiseaseWebhookData = async () => {
      const query = gql`
        query {
            kidneyDiseaseWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.kidneyDiseaseWebhooks[0]);
        setQuestionsToDisplay(getData.kidneyDiseaseWebhooks)
...
export default KidneyDiseaseWebhooks