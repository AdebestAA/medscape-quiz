...
const LymphomaTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLymphomaTwo($id: ID!) {
    deleteLymphomaTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLymphomaTwo($data: LymphomaTwoCreateInput!) {
    createLymphomaTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLymphomaTwo($id: ID!) {
    publishLymphomaTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLymphomaTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   lymphomaTwos {
    ...
    if (existingData.lymphomaTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.lymphomaTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLymphomaTwo.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchLymphomaTwoData = async () => {
      const query = gql`
        query {
            lymphomaTwos {
            id
      ...
      try {
        ...
        console.log(getData.lymphomaTwos[0]);
        setQuestionsToDisplay(getData.lymphomaTwos)
