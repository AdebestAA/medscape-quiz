...
const ChoroidalNeovascularization = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteChoroidalNeovascularization($id: ID!) {
    deleteChoroidalNeovascularization(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createChoroidalNeovascularization($data: ChoroidalNeovascularizationCreateInput!) {
    createChoroidalNeovascularization(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishChoroidalNeovascularization($id: ID!) {
    publishChoroidalNeovascularization(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createChoroidalNeovascularization.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingChoroidalNeovascularizations {
   choroidalNeovascularizations {
    id
    placeholder
    }
    }
    ...
    if (existingData.choroidalNeovascularizations.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.choroidalNeovascularizations[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishChoroidalNeovascularization.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchChoroidalNeovascularizationData = async () => {
      const query = gql`
        query {
            choroidalNeovascularizations {
            id
      ...
      try {
        ...
        console.log(getData.choroidalNeovascularizations[0]);
        setQuestionsToDisplay(getData.choroidalNeovascularizations)
   ...
   fetchChoroidalNeovascularizationData();
  }, [dataPosted]);
...
export default ChoroidalNeovascularization
