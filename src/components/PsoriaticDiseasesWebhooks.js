...
const PsoriaticDiseasesWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deletePsoriaticDiseasesWebhook($id: ID!) {
    deletePsoriaticDiseasesWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createPsoriaticDiseasesWebhook($data: PsoriaticDiseasesWebhookCreateInput!) {
    createPsoriaticDiseasesWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishPsoriaticDiseasesWebhook($id: ID!) {
    publishPsoriaticDiseasesWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createPsoriaticDiseasesWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingPsoriaticDiseasesWebhooks {
   psoriaticDiseasesWebhooks {
    id
    placeholder
    }
    }
    ...
    if (existingData.psoriaticDiseasesWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.psoriaticDiseasesWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishPsoriaticDiseasesWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchPsoriaticDiseasesWebhookData = async () => {
      const query = gql`
        query {
            psoriaticDiseasesWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.psoriaticDiseasesWebhooks[0]);
        setQuestionsToDisplay(getData.psoriaticDiseasesWebhooks)
   ...
   fetchPsoriaticDiseasesWebhookData();
  }, [dataPosted]);
...
export default PsoriaticDiseasesWebhooks