...
const HemophiliaChallenges = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteHemophiliaChallenge($id: ID!) {
    deleteHemophiliaChallenge(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHemophiliaChallenge($data: HemophiliaChallengeCreateInput!) {
    createHemophiliaChallenge(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishHemophiliaChallenge($id: ID!) {
    publishHemophiliaChallenge(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHemophiliaChallenge.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingHemophiliaChallenges {
   hemophiliaChallenges {
    ...
    if (existingData.hemophiliaChallenges.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.hemophiliaChallenges[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHemophiliaChallenge.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchHemophiliaChallengeData = async () => {
      const query = gql`
        query {
            hemophiliaChallenges {
            id
      ...
      try {
        ...
        console.log(getData.hemophiliaChallenges[0]);
        setQuestionsToDisplay(getData.hemophiliaChallenges)
...
export default HemophiliaChallenges