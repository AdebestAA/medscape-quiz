...
const NAshtma = ({dataFromTypeForm}) => {
...
overwriteAndPublishQuest(dataFromTypeForm);
 ...
 mutation deleteNAshtma($id: ID!) {
    deleteNAshtma(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createNAshtma($data: NAshtmaCreateInput!) {
    createNAshtma(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishNAshtma($id: ID!) {
    publishNAshtma(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createNAshtma.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   nAshtmas {
    ...
    if (existingData.nAshtmas.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.nAshtmas[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishNAshtma.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchMgHitData = async () => {
      ...
      try {
        ...
        console.log(getData.nAshtmas[0]);
        setQuestionsToDisplay(getData.nAshtmas)
...
// STATEMENT
 ...
 return (
    ...
    <aside ref={el => eachQuestAndFeedback.current[index] = el} key={quest?.id} className="py-16 text-lg h-screen overflow-y-scroll ">
         <div className="flex flex-col items-center justify-center w-full px-64 min-h-screen space-y-4"> 
