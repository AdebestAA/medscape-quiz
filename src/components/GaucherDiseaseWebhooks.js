...
const GaucherDiseaseWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteGaucherDiseaseWebhook($id: ID!) {
    deleteGaucherDiseaseWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createGaucherDiseaseWebhook($data: GaucherDiseaseWebhookCreateInput!) {
    createGaucherDiseaseWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishGaucherDiseaseWebhook($id: ID!) {
    publishGaucherDiseaseWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createGaucherDiseaseWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingGaucherDiseaseWebhooks {
   gaucherDiseaseWebhooks {
    ...
    if (existingData.gaucherDiseaseWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.gaucherDiseaseWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishGaucherDiseaseWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchGaucherDiseaseWebhookData = async () => {
      const query = gql`
        query {
            gaucherDiseaseWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.gaucherDiseaseWebhooks[0]);
        setQuestionsToDisplay(getData.gaucherDiseaseWebhooks)
   ...
   fetchGaucherDiseaseWebhookData();
  }, [dataPosted]);
...
export default GaucherDiseaseWebhooks