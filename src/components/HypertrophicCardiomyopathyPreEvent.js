...
const HypertrophicCardiomyopathyPreEvent = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteHypertrophicCardiomyopathyPreEvent($id: ID!) {
    deleteHypertrophicCardiomyopathyPreEvent(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHypertrophicCardiomyopathyPreEvent($data: HypertrophicCardiomyopathyPreEventCreateInput!) {
    createHypertrophicCardiomyopathyPreEvent(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishHypertrophicCardiomyopathyPreEvent($id: ID!) {
    publishHypertrophicCardiomyopathyPreEvent(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHypertrophicCardiomyopathyPreEvent.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingHypertrophicCardiomyopathyPreEvents {
  hypertrophicCardiomyopathyPreEvents {
    id
    placeholder
    }
    }
    ...
    if (existingData.hypertrophicCardiomyopathyPreEvents.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.hypertrophicCardiomyopathyPreEvents[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHypertrophicCardiomyopathyPreEvent.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchHypertrophicCardiomyopathyPreEventData = async () => {
      const query = gql`
        query {
            hypertrophicCardiomyopathyPreEvents {
            id
      ...
      try {
        ...
        console.log(getData.hypertrophicCardiomyopathyPreEvents[0]);
        setQuestionsToDisplay(getData.hypertrophicCardiomyopathyPreEvents)
   ...
   fetchHypertrophicCardiomyopathyPreEventData();
  }, [dataPosted]);
...
export default HypertrophicCardiomyopathyPreEvent