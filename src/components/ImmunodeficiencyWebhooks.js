...
const ImmunodeficiencyWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteImmunodeficiencyWebhook($id: ID!) {
    deleteImmunodeficiencyWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createImmunodeficiencyWebhook($data: ImmunodeficiencyWebhookCreateInput!) {
    createImmunodeficiencyWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishImmunodeficiencyWebhook($id: ID!) {
    publishImmunodeficiencyWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createImmunodeficiencyWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingImmunodeficiencyWebhooks {
   immunodeficiencyWebhooks {
    id
    placeholder
    }
    }
    ...
    if (existingData.immunodeficiencyWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.immunodeficiencyWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishImmunodeficiencyWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchImmunodeficiencyWebhookData = async () => {
      const query = gql`
        query {
            immunodeficiencyWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.immunodeficiencyWebhooks[0]);
        setQuestionsToDisplay(getData.immunodeficiencyWebhooks)
   ...
   fetchImmunodeficiencyWebhookData();
  }, [dataPosted]);
...
export default ImmunodeficiencyWebhooks