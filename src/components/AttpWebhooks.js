...
const AttpWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAttpWebhook($id: ID!) {
    deleteAttpWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAttpWebhook($data: AttpWebhookCreateInput!) {
    createAttpWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAttpWebhook($id: ID!) {
    publishAttpWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAttpWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   attpWebhooks {
    ...
    if (existingData.attpWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.attpWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAttpWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchAttpWebhookData = async () => {
      const query = gql`
        query {
            attpWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.attpWebhooks[0]);
        setQuestionsToDisplay(getData.attpWebhooks)
