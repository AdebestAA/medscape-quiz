...
const MultipleSclerosisCaseOne = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteMultipleSclerosisCaseOne($id: ID!) {
    deleteMultipleSclerosisCaseOne(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createMultipleSclerosisCaseOne($data: MultipleSclerosisCaseOneCreateInput!) {
    createMultipleSclerosisCaseOne(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishMultipleSclerosisCaseOne($id: ID!) {
    publishMultipleSclerosisCaseOne(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createMultipleSclerosisCaseOne.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingMultipleSclerosisCaseOnes {
  multipleSclerosisCaseOnes {
    id
    placeholder
    }
    }
    ...
    if (existingData.multipleSclerosisCaseOnes.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.multipleSclerosisCaseOnes[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishMultipleSclerosisCaseOne.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchMultipleSclerosisCaseOneData = async () => {
      const query = gql`
        query {
            multipleSclerosisCaseOnes {
            id
      ...
      try {
        ...
        console.log(getData.multipleSclerosisCaseOnes[0]);
        setQuestionsToDisplay(getData.multipleSclerosisCaseOnes)
   ...
   fetchMultipleSclerosisCaseOneData();
  }, [dataPosted]);
...
export default MultipleSclerosisCaseOne