...
const HepatocellularCarcinomaWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteHepatocellularCarcinomaWebhook($id: ID!) {
    deleteHepatocellularCarcinomaWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHepatocellularCarcinomaWebhook($data: HepatocellularCarcinomaWebhookCreateInput!) {
    createHepatocellularCarcinomaWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishHepatocellularCarcinomaWebhook($id: ID!) {
    publishHepatocellularCarcinomaWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHepatocellularCarcinomaWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingHepatocellularCarcinomaWebhooks {
   hepatocellularCarcinomaWebhooks {
    id
    placeholder
    }
    }
    ...
    if (existingData.hepatocellularCarcinomaWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.hepatocellularCarcinomaWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHepatocellularCarcinomaWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchHepatocellularCarcinomaWebhookData = async () => {
      const query = gql`
        query {
            hepatocellularCarcinomaWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.hepatocellularCarcinomaWebhooks[0]);
        setQuestionsToDisplay(getData.hepatocellularCarcinomaWebhooks)
   ...
   fetchHepatocellularCarcinomaWebhookData();
  }, [dataPosted]);
...
export default HepatocellularCarcinomaWebhooks