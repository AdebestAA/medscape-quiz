...
const Coagulation = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCoagulation($id: ID!) {
    deleteCoagulation(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCoagulation($data: CoagulationCreateInput!) {
    createCoagulation(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCoagulation($id: ID!) {
    publishCoagulation(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCoagulation.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   coagulations {
    ...
    if (existingData.coagulations.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.coagulations[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCoagulation.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCoagulationData = async () => {
      const query = gql`
        query {
            coagulations {
            id
      ...
      try {
        ...
        console.log(getData.coagulations[0]);
        setQuestionsToDisplay(getData.coagulations)
