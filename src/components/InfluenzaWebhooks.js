...
const InfluenzaWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteInfluenzaWebhook($id: ID!) {
    deleteInfluenzaWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createInfluenzaWebhook($data: InfluenzaWebhookCreateInput!) {
    createInfluenzaWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishInfluenzaWebhook($id: ID!) {
    publishInfluenzaWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createInfluenzaWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingInfluenzaWebhooks {
   influenzaWebhooks {
    id
    placeholder
    }
    ...
    if (existingData.influenzaWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.influenzaWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishInfluenzaWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchInfluenzaWebhookData = async () => {
      const query = gql`
        query {
            influenzaWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.influenzaWebhooks[0]);
        setQuestionsToDisplay(getData.influenzaWebhooks)
...
export default InfluenzaWebhooks