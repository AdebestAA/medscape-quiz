...
const DeleteDup = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteDeleteDup($id: ID!) {
    deleteDeleteDup(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createDeleteDup($data: DeleteDupCreateInput!) {
    createDeleteDup(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishDeleteDup($id: ID!) {
    publishDeleteDup(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createDeleteDup.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   deleteDups {
    ...
    if (existingData.deleteDups.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.deleteDups[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishDeleteDup.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchDeleteDupData = async () => {
      const query = gql`
        query {
            deleteDups {
            id
      ...
      try {
        ...
        console.log(getData.deleteDups[0]);
        setQuestionsToDisplay(getData.deleteDups)
