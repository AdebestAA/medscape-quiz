...
const IntracranialHemorrhageJp = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteIntracranialHemorrhageJp($id: ID!) {
    deleteIntracranialHemorrhageJp(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createIntracranialHemorrhageJp($data: IntracranialHemorrhageJpCreateInput!) {
    createIntracranialHemorrhageJp(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishIntracranialHemorrhageJp($id: ID!) {
    publishIntracranialHemorrhageJp(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createIntracranialHemorrhageJp.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingIntracranialHemorrhageJps {
   intracranialHemorrhageJps {
    id
    placeholder
    }
    }
    ...
    if (existingData.intracranialHemorrhageJps.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.intracranialHemorrhageJps[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishIntracranialHemorrhageJp.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchIntracranialHemorrhageJpData = async () => {
      const query = gql`
        query {
            intracranialHemorrhageJps {
            id
      ...
      try {
        ...
        console.log(getData.intracranialHemorrhageJps[0]);
        setQuestionsToDisplay(getData.intracranialHemorrhageJps)
   ...
   fetchIntracranialHemorrhageJpData();
  }, [dataPosted]);
...
export default IntracranialHemorrhageJp
