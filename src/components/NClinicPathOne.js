...
const NClinicPathOne = ({dataFromTypeForm}) => {
...
overwriteAndPublishQuest(dataFromTypeForm);
 ...
 mutation deleteNClinicPathOne($id: ID!) {
    deleteNClinicPathOne(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createNClinicPathOne($data: NClinicPathOneCreateInput!) {
    createNClinicPathOne(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishNClinicPathOne($id: ID!) {
    publishNClinicPathOne(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createNClinicPathOne.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingNClinicPathOnes {
   nClinicPathOnes {
    id
    placeholder
    }
    ...
    if (existingData.nClinicPathOnes.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.nClinicPathOnes[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishNClinicPathOne.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchNClinicPathOneData = async () => {
      const query = gql`
        query {
            nClinicPathOnes {
            id
      ...
      try {
        ...
        console.log(getData.nClinicPathOnes[0]);
        setQuestionsToDisplay(getData.nClinicPathOnes)
