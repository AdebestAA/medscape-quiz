...
const BreastCancerWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteBreastCancerWebhook($id: ID!) {
    deleteBreastCancerWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createBreastCancerWebhook($data: BreastCancerWebhookCreateInput!) {
    createBreastCancerWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishBreastCancerWebhook($id: ID!) {
    publishBreastCancerWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createBreastCancerWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingBreastCancerWebhooks {
   breastCancerWebhooks {
    id
    placeholder
    }
    }
    ...
    if (existingData.breastCancerWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.breastCancerWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishBreastCancerWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchBreastCancerWebhookData = async () => {
      const query = gql`
        query {
            breastCancerWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.breastCancerWebhooks[0]);
        setQuestionsToDisplay(getData.breastCancerWebhooks)
...
export default BreastCancerWebhooks