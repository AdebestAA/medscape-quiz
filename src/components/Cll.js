...
console.log(dataFromTypeForm);
 ...
 mutation deleteCll($id: ID!) {
    deleteCll(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCll($data: CllCreateInput!) {
    createCll(data: $data) {
      id
      placeholder
    }
...
const publishQuestMutation = gql`
  mutation publishCll($id: ID!) {
    publishCll(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCll.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   clls {
    ...
    if (existingData.clls.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.clls[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCll.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchCllData = async () => {
      ...
      try {
        ...
        console.log(getData.clls[0]);
        setQuestionsToDisplay(getData.clls)
