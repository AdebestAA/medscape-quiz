...
const PompeDiseaseWebhook = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deletePompeDiseaseWebhook($id: ID!) {
    deletePompeDiseaseWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createPompeDiseaseWebhook($data: PompeDiseaseWebhookCreateInput!) {
    createPompeDiseaseWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishPompeDiseaseWebhook($id: ID!) {
    publishPompeDiseaseWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createPompeDiseaseWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingPompeDiseaseWebhooks {
   pompeDiseaseWebhooks {
    id
    placeholder
    }
    }
    ...
    if (existingData.pompeDiseaseWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.pompeDiseaseWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishPompeDiseaseWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchPompeDiseaseWebhookData = async () => {
      const query = gql`
        query {
            pompeDiseaseWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.pompeDiseaseWebhooks[0]);
        setQuestionsToDisplay(getData.pompeDiseaseWebhooks)
...
export default PompeDiseaseWebhook
