...
const CytomegalovirusBiotest = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCytomegalovirusBiotest($id: ID!) {
    deleteCytomegalovirusBiotest(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCytomegalovirusBiotest($data: CytomegalovirusBiotestCreateInput!) {
    createCytomegalovirusBiotest(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCytomegalovirusBiotest($id: ID!) {
    publishCytomegalovirusBiotest(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCytomegalovirusBiotest.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCytomegalovirusBiotests {
  cytomegalovirusBiotests {
    id
    placeholder
    }
    }
    ...
    if (existingData.cytomegalovirusBiotests.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.cytomegalovirusBiotests[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCytomegalovirusBiotest.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCytomegalovirusBiotestData = async () => {
      const query = gql`
        query {
            cytomegalovirusBiotests {
            id
      ...
      try {
        ...
        console.log(getData.cytomegalovirusBiotests[0]);
        setQuestionsToDisplay(getData.cytomegalovirusBiotests)
   ...
   fetchCytomegalovirusBiotestData();
  }, [dataPosted]);
...
export default CytomegalovirusBiotest