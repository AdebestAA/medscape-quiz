...
const PolycythemiaVera = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deletePolycythemiaVera($id: ID!) {
    deletePolycythemiaVera(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createPolycythemiaVera($data: PolycythemiaVeraCreateInput!) {
    createPolycythemiaVera(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishPolycythemiaVera($id: ID!) {
    publishPolycythemiaVera(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createPolycythemiaVera.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingPolycythemiaVeras {
   polycythemiaVeras {
    id
    placeholder
    }
    ...
    if (existingData.polycythemiaVeras.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.polycythemiaVeras[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishPolycythemiaVera.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchPolycythemiaVeraData = async () => {
      const query = gql`
        query {
            polycythemiaVeras {
            id
      ...
      try {
        ...
        console.log(getData.polycythemiaVeras[0]);
        setQuestionsToDisplay(getData.polycythemiaVeras)
...
export default PolycythemiaVera