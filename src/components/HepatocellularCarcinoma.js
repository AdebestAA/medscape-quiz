...
const HepatocellularCarcinoma = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteHepatocellularCarcinoma($id: ID!) {
    deleteHepatocellularCarcinoma(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHepatocellularCarcinoma($data: HepatocellularCarcinomaCreateInput!) {
    createHepatocellularCarcinoma(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishHepatocellularCarcinoma($id: ID!) {
    publishHepatocellularCarcinoma(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHepatocellularCarcinoma.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingHepatocellularCarcinomas {
   hepatocellularCarcinomas {
    ...
    if (existingData.hepatocellularCarcinomas.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.hepatocellularCarcinomas[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHepatocellularCarcinoma.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchHepatocellularCarcinomaData = async () => {
      const query = gql`
        query {
            hepatocellularCarcinomas {
            id
      ...
      try {
        ...
        console.log(getData.hepatocellularCarcinomas[0]);
        setQuestionsToDisplay(getData.hepatocellularCarcinomas)
   ...
   fetchHepatocellularCarcinomaData();
  }, [dataPosted]);
