...
const Superscript = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteSuperscript($id: ID!) {
    deleteSuperscript(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createSuperscript($data: SuperscriptCreateInput!) {
    createSuperscript(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishSuperscript($id: ID!) {
    publishSuperscript(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createSuperscript.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   superscripts {
    ...
    if (existingData.superscripts.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.superscripts[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishSuperscript.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchSuperscriptData = async () => {
      const query = gql`
        query {
            superscripts {
            id
      ...
      try {
        ...
        console.log(getData.superscripts[0]);
        setQuestionsToDisplay(getData.superscripts)
