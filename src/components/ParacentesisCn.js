...
const ParacentesisCn = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteParacentesisCn($id: ID!) {
    deleteParacentesisCn(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createParacentesisCn($data: ParacentesisCnCreateInput!) {
    createParacentesisCn(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishParacentesisCn($id: ID!) {
    publishParacentesisCn(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createParacentesisCn.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingParacentesisCns {
   paracentesisCns {
    id
    placeholder
    }
    ...
    if (existingData.paracentesisCns.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.paracentesisCns[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishParacentesisCn.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchParacentesisCnData = async () => {
      const query = gql`
        query {
            paracentesisCns {
            id
      ...
      try {
        ...
        console.log(getData.paracentesisCns[0]);
        setQuestionsToDisplay(getData.paracentesisCns)
