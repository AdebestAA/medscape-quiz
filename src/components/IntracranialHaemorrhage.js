...
const IntracranialHaemorrhage = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteIntracranialHaemorrhage($id: ID!) {
    deleteIntracranialHaemorrhage(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createIntracranialHaemorrhage($data: IntracranialHaemorrhageCreateInput!) {
    createIntracranialHaemorrhage(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishIntracranialHaemorrhage($id: ID!) {
    publishIntracranialHaemorrhage(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createIntracranialHaemorrhage.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingIntracranialHaemorrhages {
   intracranialHaemorrhages {
    id
    placeholder
    }
    }
    ...
    if (existingData.intracranialHaemorrhages.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.intracranialHaemorrhages[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishIntracranialHaemorrhage.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchIntracranialHaemorrhageData = async () => {
      const query = gql`
        query {
            intracranialHaemorrhages {
            id
      ...
      try {
        ...
        console.log(getData.intracranialHaemorrhages[0]);
        setQuestionsToDisplay(getData.intracranialHaemorrhages)
   ...
   fetchIntracranialHaemorrhageData();
  }, [dataPosted]);
...
export default IntracranialHaemorrhage
