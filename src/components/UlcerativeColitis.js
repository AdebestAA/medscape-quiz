...
const UlcerativeColitis = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteUlcerativeColiti($id: ID!) {
    deleteUlcerativeColiti(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createUlcerativeColiti($data: UlcerativeColitiCreateInput!) {
    createUlcerativeColiti(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishUlcerativeColiti($id: ID!) {
    publishUlcerativeColiti(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createUlcerativeColiti.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingUlcerativeColitis {
   ulcerativeColitis {
    id
    placeholder
    }
    ...
    if (existingData.ulcerativeColitis.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.ulcerativeColitis[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishUlcerativeColiti.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchUlcerativeColitiData = async () => {
      const query = gql`
        query {
            ulcerativeColitis {
            id
      ...
      try {
        ...
        console.log(getData.ulcerativeColitis[0]);
        setQuestionsToDisplay(getData.ulcerativeColitis)
