...
const NClinicPathTwo = ({dataFromTypeForm}) => {
...
overwriteAndPublishQuest(dataFromTypeForm);
 ...
 mutation deleteNClinicPathTwo($id: ID!) {
    deleteNClinicPathTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createNClinicPathTwo($data: NClinicPathTwoCreateInput!) {
    createNClinicPathTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishNClinicPathTwo($id: ID!) {
    publishNClinicPathTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createNClinicPathTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingNClinicPathTwos {
   nClinicPathTwos {
    id
    placeholder
    }
    ...
    if (existingData.nClinicPathTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.nClinicPathTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishNClinicPathTwo.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchNClinicPathTwoData = async () => {
      const query = gql`
        query {
            nClinicPathTwos {
            id
      ...
      try {
        ...
        console.log(getData.nClinicPathTwos[0]);
        setQuestionsToDisplay(getData.nClinicPathTwos)
