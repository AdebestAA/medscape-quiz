...
const Parkinson = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteParkinson($id: ID!) {
    deleteParkinson(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createParkinson($data: ParkinsonCreateInput!) {
    createParkinson(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishParkinson($id: ID!) {
    publishParkinson(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createParkinson.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   parkinsons {
    ...
    if (existingData.parkinsons.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.parkinsons[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishParkinson.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchParkinsonData = async () => {
      const query = gql`
        query {
            parkinsons {
            id
      ...
      try {
        ...
        console.log(getData.parkinsons[0]);
        setQuestionsToDisplay(getData.parkinsons)
