...
const NsclcWebhooksTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteNsclcWebhooksTwo($id: ID!) {
    deleteNsclcWebhooksTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createNsclcWebhooksTwo($data: NsclcWebhooksTwoCreateInput!) {
    createNsclcWebhooksTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishNsclcWebhooksTwo($id: ID!) {
    publishNsclcWebhooksTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createNsclcWebhooksTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingNsclcWebhooksTwos {
   nsclcWebhooksTwos {
    id
    placeholder
    }
    ...
    if (existingData.nsclcWebhooksTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.nsclcWebhooksTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishNsclcWebhooksTwo.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchNsclcWebhooksTwoData = async () => {
      const query = gql`
        query {
            nsclcWebhooksTwos {
            id
      ...
      try {
        ...
        console.log(getData.nsclcWebhooksTwos[0]);
        setQuestionsToDisplay(getData.nsclcWebhooksTwos)
...
export default NsclcWebhooksTwo