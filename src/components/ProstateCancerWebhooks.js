...
const ProstateCancerWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteProstateCancerWebhook($id: ID!) {
    deleteProstateCancerWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createProstateCancerWebhook($data: ProstateCancerWebhookCreateInput!) {
    createProstateCancerWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishProstateCancerWebhook($id: ID!) {
    publishProstateCancerWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createProstateCancerWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingProstateCancerWebhooks {
   prostateCancerWebhooks {
    id
    placeholder
    }
    }
    ...
    if (existingData.prostateCancerWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.prostateCancerWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishProstateCancerWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchProstateCancerWebhookData = async () => {
      const query = gql`
        query {
            prostateCancerWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.prostateCancerWebhooks[0]);
        setQuestionsToDisplay(getData.prostateCancerWebhooks)
   ...
   fetchProstateCancerWebhookData();
  }, [dataPosted]);
...
export default ProstateCancerWebhooks