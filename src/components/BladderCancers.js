...
const BladderCancers = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteBladderCancer($id: ID!) {
    deleteBladderCancer(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createBladderCancer($data: BladderCancerCreateInput!) {
    createBladderCancer(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishBladderCancer($id: ID!) {
    publishBladderCancer(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createBladderCancer.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingBladderCancers {
   bladderCancers {
    id
    placeholder
    }
    ...
    if (existingData.bladderCancers.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.bladderCancers[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishBladderCancer.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchBladderCancerData = async () => {
      const query = gql`
        query {
            bladderCancers {
            id
      ...
      try {
        ...
        console.log(getData.bladderCancers[0]);
        setQuestionsToDisplay(getData.bladderCancers)
