...
overwriteAndPublishQuest(dataFromTypeForm);
 ...
 mutation deleteNPain($id: ID!) {
    deleteNPain(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createNPain($data: NPainCreateInput!) {
    createNPain(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishNPain($id: ID!) {
    publishNPain(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createNPain.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   nPains {
    ...
    if (existingData.nPains.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.nPains[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishNPain.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchNPainData = async () => {
      ...
      try {
        ...
        console.log(getData.nPains[0]);
        setQuestionsToDisplay(getData.nPains)
