...
const HemophiliaWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteHemophiliaWebhook($id: ID!) {
    deleteHemophiliaWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHemophiliaWebhook($data: HemophiliaWebhookCreateInput!) {
    createHemophiliaWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishHemophiliaWebhook($id: ID!) {
    publishHemophiliaWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHemophiliaWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingHemophiliaWebhooks {
   hemophiliaWebhooks {
    id
    placeholder
    }
    ...
    if (existingData.hemophiliaWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.hemophiliaWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHemophiliaWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchHemophiliaWebhookData = async () => {
      const query = gql`
        query {
            hemophiliaWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.hemophiliaWebhooks[0]);
        setQuestionsToDisplay(getData.hemophiliaWebhooks)
...
export default HemophiliaWebhooks