...
const AcuteMyeloidLeukaemiaWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAcuteMyeloidLeukaemiaWebhook($id: ID!) {
    deleteAcuteMyeloidLeukaemiaWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAcuteMyeloidLeukaemiaWebhook($data: AcuteMyeloidLeukaemiaWebhookCreateInput!) {
    createAcuteMyeloidLeukaemiaWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAcuteMyeloidLeukaemiaWebhook($id: ID!) {
    publishAcuteMyeloidLeukaemiaWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAcuteMyeloidLeukaemiaWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingAcuteMyeloidLeukaemiaWebhooks {
   acuteMyeloidLeukaemiaWebhooks {
    ...
    if (existingData.acuteMyeloidLeukaemiaWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.acuteMyeloidLeukaemiaWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAcuteMyeloidLeukaemiaWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchAcuteMyeloidLeukaemiaWebhookData = async () => {
      const query = gql`
        query {
            acuteMyeloidLeukaemiaWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.acuteMyeloidLeukaemiaWebhooks[0]);
        setQuestionsToDisplay(getData.acuteMyeloidLeukaemiaWebhooks)
   ...
   fetchAcuteMyeloidLeukaemiaWebhookData();
  }, [dataPosted]);
...
export default AcuteMyeloidLeukaemiaWebhooks