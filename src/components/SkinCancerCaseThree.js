...
const SkinCancerCaseThree = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteSkinCancerCaseThree($id: ID!) {
    deleteSkinCancerCaseThree(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createSkinCancerCaseThree($data: SkinCancerCaseThreeCreateInput!) {
    createSkinCancerCaseThree(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishSkinCancerCaseThree($id: ID!) {
    publishSkinCancerCaseThree(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createSkinCancerCaseThree.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingSkinCancerCaseThrees {
   skinCancerCaseThrees {
    id
    placeholder
    }
    }
    ...
    if (existingData.skinCancerCaseThrees.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.skinCancerCaseThrees[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishSkinCancerCaseThree.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchSkinCancerCaseThreeData = async () => {
      const query = gql`
        query {
            skinCancerCaseThrees {
            id
      ...
      try {
        ...
        console.log(getData.skinCancerCaseThrees[0]);
        setQuestionsToDisplay(getData.skinCancerCaseThrees)
...
export default SkinCancerCaseThree
