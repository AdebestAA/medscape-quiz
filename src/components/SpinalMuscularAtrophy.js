...
const SpinalMuscularAtrophy = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteSpinalMuscularAtrophy($id: ID!) {
    deleteSpinalMuscularAtrophy(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createSpinalMuscularAtrophy($data: SpinalMuscularAtrophyCreateInput!) {
    createSpinalMuscularAtrophy(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishSpinalMuscularAtrophy($id: ID!) {
    publishSpinalMuscularAtrophy(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createSpinalMuscularAtrophy.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingSpinalMuscularAtrophies {
   spinalMuscularAtrophies {
    id
    placeholder
    }
    }
    ...
    if (existingData.spinalMuscularAtrophies.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.spinalMuscularAtrophies[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishSpinalMuscularAtrophy.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchSpinalMuscularAtrophyData = async () => {
      const query = gql`
        query {
            spinalMuscularAtrophies {
            id
      ...
      try {
        ...
        console.log(getData. spinalMuscularAtrophies[0]);
        setQuestionsToDisplay(getData. spinalMuscularAtrophies)
   ...
   fetchSpinalMuscularAtrophyData();
  }, [dataPosted]);
...
export default SpinalMuscularAtrophy