...
const LeukemiaCaseOne = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLeukemiaCaseOne($id: ID!) {
    deleteLeukemiaCaseOne(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLeukemiaCaseOne($data: LeukemiaCaseOneCreateInput!) {
    createLeukemiaCaseOne(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLeukemiaCaseOne($id: ID!) {
    publishLeukemiaCaseOne(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLeukemiaCaseOne.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingLeukemiaCaseOnes {
  leukemiaCaseOnes {
    id
    placeholder
    }
    ...
    if (existingData.leukemiaCaseOnes.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.leukemiaCaseOnes[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLeukemiaCaseOne.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchLeukemiaCaseOneData = async () => {
      const query = gql`
        query {
            leukemiaCaseOnes {
            id
      ...
      try {
        ...
        console.log(getData.leukemiaCaseOnes[0]);
        setQuestionsToDisplay(getData.leukemiaCaseOnes)
...
export default LeukemiaCaseOne