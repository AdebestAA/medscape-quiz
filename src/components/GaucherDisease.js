...
const GaucherDisease = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteGaucherDisease($id: ID!) {
    deleteGaucherDisease(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createGaucherDisease($data: GaucherDiseaseCreateInput!) {
    createGaucherDisease(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishGaucherDisease($id: ID!) {
    publishGaucherDisease(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createGaucherDisease.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingGaucherDiseases {
   gaucherDiseases {
    id
    placeholder
    }
    ...
    if (existingData.gaucherDiseases.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.gaucherDiseases[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishGaucherDisease.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchGaucherDiseaseData = async () => {
      const query = gql`
        query {
            gaucherDiseases {
            id
      ...
      try {
        ...
        console.log(getData.gaucherDiseases[0]);
        setQuestionsToDisplay(getData.gaucherDiseases)
