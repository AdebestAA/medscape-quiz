...
const HematopoieticStemcellTransplantation = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteHematopoieticStemcellTransplantation($id: ID!) {
    deleteHematopoieticStemcellTransplantation(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHematopoieticStemcellTransplantation($data: HematopoieticStemcellTransplantationCreateInput!) {
    createHematopoieticStemcellTransplantation(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishHematopoieticStemcellTransplantation($id: ID!) {
    publishHematopoieticStemcellTransplantation(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHematopoieticStemcellTransplantation.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingHematopoieticStemcellTransplantations {
   hematopoieticStemcellTransplantations {
    id
    placeholder
    }
    }
    ...
    if (existingData.hematopoieticStemcellTransplantations.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.hematopoieticStemcellTransplantations[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHematopoieticStemcellTransplantation.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchHematopoieticStemcellTransplantationData = async () => {
      const query = gql`
        query {
            hematopoieticStemcellTransplantations {
            id
      ...
      try {
        ...
        console.log(getData.hematopoieticStemcellTransplantations[0]);
        setQuestionsToDisplay(getData.hematopoieticStemcellTransplantations)
   ...
   fetchHematopoieticStemcellTransplantationData();
  }, [dataPosted]);
...
export default HematopoieticStemcellTransplantation