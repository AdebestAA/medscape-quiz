...
const Paracentesis = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteParacentesi($id: ID!) {
    deleteParacentesi(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createParacentesi($data: ParacentesiCreateInput!) {
    createParacentesi(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishParacentesi($id: ID!) {
    publishParacentesi(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createParacentesi.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   paracentesis {
    ...
    if (existingData.paracentesis.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.paracentesis[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishParacentesi.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchParacentesiData = async () => {
      const query = gql`
        query {
            paracentesis {
            id
      ...
      try {
        ...
        console.log(getData.paracentesis[0]);
        setQuestionsToDisplay(getData.paracentesis)
