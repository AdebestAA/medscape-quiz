...
const CaseTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCaseTwo($id: ID!) {
    deleteCaseTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCaseTwo($data: CaseTwoCreateInput!) {
    createCaseTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCaseTwo($id: ID!) {
    publishCaseTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCaseTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   caseTwos {
    ...
    if (existingData.caseTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.caseTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCaseTwo.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchCaseTwoData = async () => {
      ...
      try {
        ...
        console.log(getData.caseTwos[0]);
        setQuestionsToDisplay(getData.caseTwos)
