...
const AnaemiaAndCkdWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAnaemiaAndCkdWebhook($id: ID!) {
    deleteAnaemiaAndCkdWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAnaemiaAndCkdWebhook($data: AnaemiaAndCkdWebhookCreateInput!) {
    createAnaemiaAndCkdWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAnaemiaAndCkdWebhook($id: ID!) {
    publishAnaemiaAndCkdWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAnaemiaAndCkdWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingAnaemiaAndCkdWebhooks {
   anaemiaAndCkdWebhooks {
    ...
    if (existingData.anaemiaAndCkdWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.anaemiaAndCkdWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAnaemiaAndCkdWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchAnaemiaAndCkdWebhookData = async () => {
      const query = gql`
        query {
            anaemiaAndCkdWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.anaemiaAndCkdWebhooks[0]);
        setQuestionsToDisplay(getData.anaemiaAndCkdWebhooks)
...
export default AnaemiaAndCkdWebhooks