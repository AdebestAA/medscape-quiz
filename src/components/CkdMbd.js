...
console.log(dataFromTypeForm);
 ...
 mutation deleteCkdMbd($id: ID!) {
    deleteCkdMbd(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCkdMbd($data: CkdMbdCreateInput!) {
    createCkdMbd(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCkdMbd($id: ID!) {
    publishCkdMbd(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCkdMbd.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   ckdMbds {
    ...
    if (existingData.ckdMbds.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.ckdMbds[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCkdMbd.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchCkdMbdData = async () => {
      ...
      try {
        ...
        console.log(getData.ckdMbds[0]);
        setQuestionsToDisplay(getData.ckdMbds)
