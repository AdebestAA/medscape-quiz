...
const LymphomaThree = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLymphomaThree($id: ID!) {
    deleteLymphomaThree(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLymphomaThree($data: LymphomaThreeCreateInput!) {
    createLymphomaThree(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLymphomaThree($id: ID!) {
    publishLymphomaThree(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLymphomaThree.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingLymphomaThrees {
   lymphomaThrees {
    id
    placeholder
    }
    ...
    if (existingData.lymphomaThrees.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.lymphomaThrees[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLymphomaThree.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchLymphomaThreeData = async () => {
      const query = gql`
        query {
            lymphomaThrees {
            id
      ...
      try {
        ...
        console.log(getData.lymphomaThrees[0]);
        setQuestionsToDisplay(getData.lymphomaThrees)
