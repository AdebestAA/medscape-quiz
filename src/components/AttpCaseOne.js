...
const AttpCaseOne = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAttpCaseOne($id: ID!) {
    deleteAttpCaseOne(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAttpCaseOne($data: AttpCaseOneCreateInput!) {
    createAttpCaseOne(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAttpCaseOne($id: ID!) {
    publishAttpCaseOne(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAttpCaseOne.id;
...
const overwriteAndPublishQuest = async (newData) => {
  ...
  attpCaseOnes {
    ...
    if (existingData.attpCaseOnes.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.attpCaseOnes[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAttpCaseOne.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchAttpCaseOneData = async () => {
      const query = gql`
        query {
            attpCaseOnes {
            id
      ...
      try {
        ...
        console.log(getData.attpCaseOnes[0]);
        setQuestionsToDisplay(getData.attpCaseOnes)
