...
const AnaemiaAndCkd = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAnaemiaAndCkd($id: ID!) {
    deleteAnaemiaAndCkd(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAnaemiaAndCkd($data: AnaemiaAndCkdCreateInput!) {
    createAnaemiaAndCkd(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAnaemiaAndCkd($id: ID!) {
    publishAnaemiaAndCkd(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAnaemiaAndCkd.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingAnaemiaAndCkds {
  anaemiaAndCkds {
    id
    placeholder
    }
    ...
    if (existingData.anaemiaAndCkds.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.anaemiaAndCkds[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAnaemiaAndCkd.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchAnaemiaAndCkdData = async () => {
      const query = gql`
        query {
            anaemiaAndCkds {
            id
      ...
      try {
        ...
        console.log(getData.anaemiaAndCkds[0]);
        setQuestionsToDisplay(getData.anaemiaAndCkds)
