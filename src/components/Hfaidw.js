...
console.log(dataFromTypeForm);
 ...
 mutation deleteHfaidw($id: ID!) {
    deleteHfaidw(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHfaidw($data: HfaidwCreateInput!) {
    createHfaidw(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishHfaidw($id: ID!) {
    publishHfaidw(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHfaidw.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   hfaidws {
    ...
    if (existingData.hfaidws.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.hfaidws[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHfaidw.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchHfaidwData = async () => {
      ...
      try {
        ...
        console.log(getData.hfaidws[0]);
        setQuestionsToDisplay(getData.hfaidws)
