...
const CytomegalovirusWebhooksTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCytomegalovirusWebhooksTwo($id: ID!) {
    deleteCytomegalovirusWebhooksTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCytomegalovirusWebhooksTwo($data: CytomegalovirusWebhooksTwoCreateInput!) {
    createCytomegalovirusWebhooksTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCytomegalovirusWebhooksTwo($id: ID!) {
    publishCytomegalovirusWebhooksTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCytomegalovirusWebhooksTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCytomegalovirusWebhooksTwos {
   cytomegalovirusWebhooksTwos {
    id
    placeholder
    }
    }
    ...
    if (existingData.cytomegalovirusWebhooksTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.cytomegalovirusWebhooksTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCytomegalovirusWebhooksTwo.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCytomegalovirusWebhooksTwoData = async () => {
      const query = gql`
        query {
            cytomegalovirusWebhooksTwos {
            id
      ...
      try {
        ...
        console.log(getData.cytomegalovirusWebhooksTwos[0]);
        setQuestionsToDisplay(getData.cytomegalovirusWebhooksTwos)
   ...
   fetchCytomegalovirusWebhooksTwoData();
  }, [dataPosted]);
...
export default CytomegalovirusWebhooksTwo