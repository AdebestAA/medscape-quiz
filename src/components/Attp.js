...
console.log(dataFromTypeForm);
 ...
 mutation deleteAttp($id: ID!) {
    deleteAttp(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAttp($data: AttpCreateInput!) {
    createAttp(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAttp($id: ID!) {
    publishAttp(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAttp.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   attps {
    ...
    if (existingData.attps.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.attps[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAttp.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchAttpData = async () => {
      ...
      try {
        ...
        console.log(getData.attps[0]);
        setQuestionsToDisplay(getData.attps)
