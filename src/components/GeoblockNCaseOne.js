...
const GeoblockNCaseOne = ({dataFromTypeForm}) => {
...
overwriteAndPublishQuest(dataFromTypeForm);
 ...
 mutation deleteGeoblockNCaseOne($id: ID!) {
    deleteGeoblockNCaseOne(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createGeoblockNCaseOne($data: GeoblockNCaseOneCreateInput!) {
    createGeoblockNCaseOne(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishGeoblockNCaseOne($id: ID!) {
    publishGeoblockNCaseOne(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createGeoblockNCaseOne.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingGeoblockNCaseOnes {
   geoblockNCaseOnes {
    id
    placeholder
    }
    ...
    if (existingData.geoblockNCaseOnes.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.geoblockNCaseOnes[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishGeoblockNCaseOne.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchGeoblockNCaseOneData = async () => {
      const query = gql`
        query {
            geoblockNCaseOnes {
            id
      ...
      try {
        ...
        console.log(getData.geoblockNCaseOnes[0]);
        setQuestionsToDisplay(getData.geoblockNCaseOnes)
...
export default GeoblockNCaseOne
