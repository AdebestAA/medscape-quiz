...
const BreastLungCancer = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteBreastLungCancer($id: ID!) {
    deleteBreastLungCancer(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createBreastLungCancer($data: BreastLungCancerCreateInput!) {
    createBreastLungCancer(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishBreastLungCancer($id: ID!) {
    publishBreastLungCancer(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createBreastLungCancer.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingBreastLungCancers {
  breastLungCancers {
    id
    placeholder
    }
    ...
    if (existingData.breastLungCancers.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.breastLungCancers[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishBreastLungCancer.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchBreastLungCancerData = async () => {
      const query = gql`
        query {
            breastLungCancers {
            id
      ...
      try {
        ...
        console.log(getData.breastLungCancers[0]);
        setQuestionsToDisplay(getData.breastLungCancers)
...
export default BreastLungCancer