...
const NAhusCaseOne = ({dataFromTypeForm}) => {
...
overwriteAndPublishQuest(dataFromTypeForm);
 ...
 mutation deleteNAhusCaseOne($id: ID!) {
    deleteNAhusCaseOne(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createNAhusCaseOne($data: NAhusCaseOneCreateInput!) {
    createNAhusCaseOne(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishNAhusCaseOne($id: ID!) {
    publishNAhusCaseOne(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createNAhusCaseOne.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingNAhusCaseOnes {
   nAhusCaseOnes {
    id
    placeholder
    }
    ...
    if (existingData.nAhusCaseOnes.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.nAhusCaseOnes[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishNAhusCaseOne.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchNAhusCaseOneData = async () => {
      const query = gql`
        query {
            nAhusCaseOnes {
            id
      ...
      try {
        ...
        console.log(getData.nAhusCaseOnes[0]);
        setQuestionsToDisplay(getData.nAhusCaseOnes)
