...
const LungCancerWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLungCancerWebhook($id: ID!) {
    deleteLungCancerWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLungCancerWebhook($data: LungCancerWebhookCreateInput!) {
    createLungCancerWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLungCancerWebhook($id: ID!) {
    publishLungCancerWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLungCancerWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingLungCancerWebhooks {
   lungCancerWebhooks {
    id
    placeholder
    }
    ...
    if (existingData.lungCancerWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.lungCancerWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLungCancerWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchLungCancerWebhookData = async () => {
      const query = gql`
        query {
            lungCancerWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.lungCancerWebhooks[0]);
        setQuestionsToDisplay(getData.lungCancerWebhooks)
...
export default LungCancerWebhooks