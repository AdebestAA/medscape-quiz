...
const BreastLungCancerWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteBreastLungCancerWebhook($id: ID!) {
    deleteBreastLungCancerWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createBreastLungCancerWebhook($data: BreastLungCancerWebhookCreateInput!) {
    createBreastLungCancerWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishBreastLungCancerWebhook($id: ID!) {
    publishBreastLungCancerWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createBreastLungCancerWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingBreastLungCancerWebhooks {
   breastLungCancerWebhooks {
    id
    placeholder
    }
    }
    ...
    if (existingData.breastLungCancerWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.breastLungCancerWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishBreastLungCancerWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchBreastLungCancerWebhookData = async () => {
      const query = gql`
        query {
            breastLungCancerWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.breastLungCancerWebhooks[0]);
        setQuestionsToDisplay(getData.breastLungCancerWebhooks)
   ...
   fetchBreastLungCancerWebhookData();
  }, [dataPosted]);
...
export default BreastLungCancerWebhooks