...
const InfluenzaTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteInfluenzaTwo($id: ID!) {
    deleteInfluenzaTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createInfluenzaTwo($data: InfluenzaTwoCreateInput!) {
    createInfluenzaTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishInfluenzaTwo($id: ID!) {
    publishInfluenzaTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createInfluenzaTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingInfluenzaTwos {
   influenzaTwos {
    id
    placeholder
    }
    ...
    if (existingData.influenzaTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.influenzaTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishInfluenzaTwo.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchInfluenzaTwoData = async () => {
      const query = gql`
        query {
           influenzaTwos {
            id
      ...
      try {
        ...
        console.log(getData.influenzaTwos[0]);
        setQuestionsToDisplay(getData.influenzaTwos)
