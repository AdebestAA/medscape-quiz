...
const MultipleSclerosisCaseTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteMultipleSclerosisCaseTwo($id: ID!) {
    deleteMultipleSclerosisCaseTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createMultipleSclerosisCaseTwo($data: MultipleSclerosisCaseTwoCreateInput!) {
    createMultipleSclerosisCaseTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishMultipleSclerosisCaseTwo($id: ID!) {
    publishMultipleSclerosisCaseTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createMultipleSclerosisCaseTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingMultipleSclerosisCaseTwos {
  multipleSclerosisCaseTwos {
    id
    placeholder
    }
    }
    ...
    if (existingData.multipleSclerosisCaseTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.multipleSclerosisCaseTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishMultipleSclerosisCaseTwo.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchMultipleSclerosisCaseTwoData = async () => {
      const query = gql`
        query {
            multipleSclerosisCaseTwos {
            id
      ...
      try {
        ...
        console.log(getData.multipleSclerosisCaseTwos[0]);
        setQuestionsToDisplay(getData.multipleSclerosisCaseTwos)
   ...
   fetchMultipleSclerosisCaseTwoData();
  }, [dataPosted]);
...
export default MultipleSclerosisCaseTwo