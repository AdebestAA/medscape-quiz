...
const PolycythemiaVeraWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deletePolycythemiaVeraWebhook($id: ID!) {
    deletePolycythemiaVeraWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createPolycythemiaVeraWebhook($data: PolycythemiaVeraWebhookCreateInput!) {
    createPolycythemiaVeraWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishPolycythemiaVeraWebhook($id: ID!) {
    publishPolycythemiaVeraWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createPolycythemiaVeraWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingPolycythemiaVeraWebhooks {
   polycythemiaVeraWebhooks {
    id
    placeholder
    }
    }
    ...
    if (existingData.polycythemiaVeraWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.polycythemiaVeraWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishPolycythemiaVeraWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchPolycythemiaVeraWebhookData = async () => {
      const query = gql`
        query {
            polycythemiaVeraWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.polycythemiaVeraWebhooks[0]);
        setQuestionsToDisplay(getData.polycythemiaVeraWebhooks)
   ...
   fetchPolycythemiaVeraWebhookData();
  }, [dataPosted]);
...
export default PolycythemiaVeraWebhooks