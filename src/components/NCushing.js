...
const NCushing = ({dataFromTypeForm}) => {
...
overwriteAndPublishQuest(dataFromTypeForm);
 ...
 mutation deleteNCushing($id: ID!) {
    deleteNCushing(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createNCushing($data: NCushingCreateInput!) {
    createNCushing(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishNCushing($id: ID!) {
    publishNCushing(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createNCushing.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   nCushings {
    ...
    if (existingData.nCushings.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.nCushings[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishNCushing.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchNCushingData = async () => {
      ...
      try {
        ...
        console.log(getData.nCushings[0]);
        setQuestionsToDisplay(getData.nCushings)
