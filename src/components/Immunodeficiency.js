...
const Immunodeficiency = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteImmunodeficiency($id: ID!) {
    deleteImmunodeficiency(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createImmunodeficiency($data: ImmunodeficiencyCreateInput!) {
    createImmunodeficiency(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishImmunodeficiency($id: ID!) {
    publishImmunodeficiency(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createImmunodeficiency.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingImmunodeficiencies {
   immunodeficiencies {
    id
    placeholder
    }
    ...
    if (existingData.immunodeficiencies.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.immunodeficiencies[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishImmunodeficiency.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchImmunodeficiencyData = async () => {
      const query = gql`
        query {
            immunodeficiencies {
            id
      ...
      try {
        ...
        console.log(getData.immunodeficiencies[0]);
        setQuestionsToDisplay(getData.immunodeficiencies)
...
export default Immunodeficiency