...
const AatdWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAatdWebhook($id: ID!) {
    deleteAatdWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAatdWebhook($data: AatdWebhookCreateInput!) {
    createAatdWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAatdWebhook($id: ID!) {
    publishAatdWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAatdWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   aatdWebhooks {
    ...
    if (existingData.aatdWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.aatdWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAatdWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchAatdWebhookData = async () => {
      const query = gql`
        query {
            aatdWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.aatdWebhooks[0]);
        setQuestionsToDisplay(getData.aatdWebhooks)
