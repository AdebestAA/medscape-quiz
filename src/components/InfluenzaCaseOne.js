...
const InfluenzaCaseOne = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteInfluenzaCaseOne($id: ID!) {
    deleteInfluenzaCaseOne(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createInfluenzaCaseOne($data: InfluenzaCaseOneCreateInput!) {
    createInfluenzaCaseOne(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishInfluenzaCaseOne($id: ID!) {
    publishInfluenzaCaseOne(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createInfluenzaCaseOne.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingInfluenzaCaseOnes {
  influenzaCaseOnes {
    id
    placeholder
    }
    ...
    if (existingData.influenzaCaseOnes.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.influenzaCaseOnes[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishInfluenzaCaseOne.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchInfluenzaCaseOneData = async () => {
      const query = gql`
        query {
           influenzaCaseOnes {
            id
      ...
      try {
        ...
        console.log(getData.influenzaCaseOnes[0]);
        setQuestionsToDisplay(getData.influenzaCaseOnes)
...
export default InfluenzaCaseOne