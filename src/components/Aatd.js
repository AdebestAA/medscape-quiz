...
console.log(dataFromTypeForm);
 ...
 mutation deleteAatd($id: ID!) {
    deleteAatd(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAatd($data: AatdCreateInput!) {
    createAatd(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAatd($id: ID!) {
    publishAatd(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAatd.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   aatds {
    ...
    if (existingData.aatds.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.aatds[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAatd.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchAatdData = async () => {
      ...
      try {
        ...
        console.log(getData.aatds[0]);
        setQuestionsToDisplay(getData.aatds)
