...
const CaseFour = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCaseFour($id: ID!) {
    deleteCaseFour(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCaseFour($data: CaseFourCreateInput!) {
    createCaseFour(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCaseFour($id: ID!) {
    publishCaseFour(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCaseFour.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   caseFours {
    ...
    if (existingData.caseFours.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.caseFours[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCaseFour.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchCaseFourData = async () => {
      ...
      try {
        ...
        console.log(getData.caseFours[0]);
        setQuestionsToDisplay(getData.caseFours)
