...
const CkdTdTwoPreSurveyPlusWebinar = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCkdTdTwoPreSurveyPlusWebinar($id: ID!) {
    deleteCkdTdTwoPreSurveyPlusWebinar(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCkdTdTwoPreSurveyPlusWebinar($data: CkdTdTwoPreSurveyPlusWebinarCreateInput!) {
    createCkdTdTwoPreSurveyPlusWebinar(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCkdTdTwoPreSurveyPlusWebinar($id: ID!) {
    publishCkdTdTwoPreSurveyPlusWebinar(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCkdTdTwoPreSurveyPlusWebinar.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCkdTdTwoPreSurveyPlusWebinars {
  ckdTdTwoPreSurveyPlusWebinars {
    id
    placeholder
    }
    }
    ...
    if (existingData.ckdTdTwoPreSurveyPlusWebinars.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.ckdTdTwoPreSurveyPlusWebinars[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCkdTdTwoPreSurveyPlusWebinar.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCkdTdTwoPreSurveyPlusWebinarData = async () => {
      const query = gql`
        query {
            ckdTdTwoPreSurveyPlusWebinars {
            id
      ...
      try {
        ...
        console.log(getData.ckdTdTwoPreSurveyPlusWebinars[0]);
        setQuestionsToDisplay(getData.ckdTdTwoPreSurveyPlusWebinars)
   ...
   fetchCkdTdTwoPreSurveyPlusWebinarData();
  }, [dataPosted]);
...
export default CkdTdTwoPreSurveyPlusWebinar