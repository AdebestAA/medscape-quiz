...
const RefractoryEpilepsy = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteRefractoryEpilepsy($id: ID!) {
    deleteRefractoryEpilepsy(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createRefractoryEpilepsy($data: RefractoryEpilepsyCreateInput!) {
    createRefractoryEpilepsy(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishRefractoryEpilepsy($id: ID!) {
    publishRefractoryEpilepsy(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createRefractoryEpilepsy.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingRefractoryEpilepsies {
   refractoryEpilepsies {
    id
    placeholder
    }
    }
    ...
    if (existingData.refractoryEpilepsies.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.refractoryEpilepsies[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishRefractoryEpilepsy.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchRefractoryEpilepsyData = async () => {
      const query = gql`
        query {
            refractoryEpilepsies {
            id
      ...
      try {
        ...
        console.log(getData.refractoryEpilepsies[0]);
        setQuestionsToDisplay(getData.refractoryEpilepsies)
...
export default RefractoryEpilepsy
