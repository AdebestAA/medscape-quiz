...
const NZeroSixAsthma = ({dataFromTypeForm}) => {
...
overwriteAndPublishQuest(dataFromTypeForm);
 ...
 mutation deleteNZeroSixAsthma($id: ID!) {
    deleteNZeroSixAsthma(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createNZeroSixAsthma($data: NZeroSixAsthmaCreateInput!) {
    createNZeroSixAsthma(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishNZeroSixAsthma($id: ID!) {
    publishNZeroSixAsthma(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createNZeroSixAsthma.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingNZeroSixAsthmas {
   nZeroSixAsthmas {
    id
    placeholder
    }
    ...
    if (existingData.nZeroSixAsthmas.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.nZeroSixAsthmas[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishNZeroSixAsthma.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchNZeroSixAsthmaData = async () => {
      const query = gql`
        query {
            nZeroSixAsthmas {
            id
      ...
      try {
        ...
        console.log(getData.nZeroSixAsthmas[0]);
        setQuestionsToDisplay(getData.nZeroSixAsthmas)
