...
const CkdTdTwoWebhookForCases = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCkdTdTwoWebhookForCase($id: ID!) {
    deleteCkdTdTwoWebhookForCase(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCkdTdTwoWebhookForCase($data: CkdTdTwoWebhookForCaseCreateInput!) {
    createCkdTdTwoWebhookForCase(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCkdTdTwoWebhookForCase($id: ID!) {
    publishCkdTdTwoWebhookForCase(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCkdTdTwoWebhookForCase.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCkdTdTwoWebhookForCases {
   ckdTdTwoWebhookForCases {
    ...
    if (existingData.ckdTdTwoWebhookForCases.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.ckdTdTwoWebhookForCases[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCkdTdTwoWebhookForCase.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCkdTdTwoWebhookForCaseData = async () => {
      const query = gql`
        query {
            ckdTdTwoWebhookForCases {
            id
      ...
      try {
        ...
        console.log(getData.ckdTdTwoWebhookForCases[0]);
        setQuestionsToDisplay(getData.ckdTdTwoWebhookForCases)
   ...
   fetchCkdTdTwoWebhookForCaseData();
  }, [dataPosted]);
...
export default CkdTdTwoWebhookForCases