...
const Anticoagulation = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAnticoagulation($id: ID!) {
    deleteAnticoagulation(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAnticoagulation($data: AnticoagulationCreateInput!) {
    createAnticoagulation(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAnticoagulation($id: ID!) {
    publishAnticoagulation(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAnticoagulation.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingAnticoagulations {
   anticoagulations {
    id
    placeholder
    }
    ...
    if (existingData.anticoagulations.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.anticoagulations[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAnticoagulation.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchAnticoagulationData = async () => {
      const query = gql`
        query {
            anticoagulations {
            id
      ...
      try {
        ...
        console.log(getData.anticoagulations[0]);
        setQuestionsToDisplay(getData.anticoagulations)
