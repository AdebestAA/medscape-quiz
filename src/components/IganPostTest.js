...
const IganPostTest = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteIganPostTest($id: ID!) {
    deleteIganPostTest(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createIganPostTest($data: IganPostTestCreateInput!) {
    createIganPostTest(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishIganPostTest($id: ID!) {
    publishIganPostTest(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createIganPostTest.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingIganPostTests {
   iganPostTests {
    id
    placeholder
    }
    ...
    if (existingData.iganPostTests.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.iganPostTests[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishIganPostTest.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchIganPostTestData = async () => {
      const query = gql`
        query {
            iganPostTests {
            id
      ...
      try {
        ...
        console.log(getData.iganPostTests[0]);
        setQuestionsToDisplay(getData.iganPostTests)
