...
const FibryDisease = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteFibryDisease($id: ID!) {
    deleteFibryDisease(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createFibryDisease($data: FibryDiseaseCreateInput!) {
    createFibryDisease(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishFibryDisease($id: ID!) {
    publishFibryDisease(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createFibryDisease.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingFibryDiseases {
   fibryDiseases {
    id
    placeholder
    }
    ...
    if (existingData.fibryDiseases.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.fibryDiseases[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishFibryDisease.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchFibryDiseaseData = async () => {
      const query = gql`
        query {
            fibryDiseases {
            id
      ...
      try {
        ...
        console.log(getData.fibryDiseases[0]);
        setQuestionsToDisplay(getData.fibryDiseases)
