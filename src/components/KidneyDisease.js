...
const KidneyDisease = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteKidneyDisease($id: ID!) {
    deleteKidneyDisease(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createKidneyDisease($data: KidneyDiseaseCreateInput!) {
    createKidneyDisease(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishKidneyDisease($id: ID!) {
    publishKidneyDisease(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createKidneyDisease.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingKidneyDiseases {
   kidneyDiseases {
    id
    placeholder
    }
    ...
    if (existingData.kidneyDiseases.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.kidneyDiseases[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishKidneyDisease.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchKidneyDiseaseData = async () => {
      const query = gql`
        query {
            kidneyDiseases {
            id
      ...
      try {
        ...
        console.log(getData.kidneyDiseases[0]);
        setQuestionsToDisplay(getData.kidneyDiseases)
