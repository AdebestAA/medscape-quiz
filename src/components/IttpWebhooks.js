...
const IttpWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteIttpWebhook($id: ID!) {
    deleteIttpWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createIttpWebhook($data: IttpWebhookCreateInput!) {
    createIttpWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishIttpWebhook($id: ID!) {
    publishIttpWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createIttpWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   ittpWebhooks {
    ...
    if (existingData.ittpWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.ittpWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishIttpWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchIttpWebhookData = async () => {
      const query = gql`
        query {
            ittpWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.ittpWebhooks[0]);
        setQuestionsToDisplay(getData.ittpWebhooks)
