...
const ChronicCough = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteChronicCough($id: ID!) {
    deleteChronicCough(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createChronicCough($data: ChronicCoughCreateInput!) {
    createChronicCough(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishChronicCough($id: ID!) {
    publishChronicCough(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createChronicCough.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingChronicCoughs {
   chronicCoughs {
    id
    placeholder
    }
    ...
    if (existingData.chronicCoughs.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.chronicCoughs[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishChronicCough.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchChronicCoughData = async () => {
      const query = gql`
        query {
            chronicCoughs {
            id
      ...
      try {
        ...
        console.log(getData.chronicCoughs[0]);
        setQuestionsToDisplay(getData.chronicCoughs)
