...
const AnaemiaAndCkdImpactAssessment = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAnaemiaAndCkdImpactAssessment($id: ID!) {
    deleteAnaemiaAndCkdImpactAssessment(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAnaemiaAndCkdImpactAssessment($data: AnaemiaAndCkdImpactAssessmentCreateInput!) {
    createAnaemiaAndCkdImpactAssessment(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAnaemiaAndCkdImpactAssessment($id: ID!) {
    publishAnaemiaAndCkdImpactAssessment(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAnaemiaAndCkdImpactAssessment.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingAnaemiaAndCkdImpactAssessments {
  anaemiaAndCkdImpactAssessments {
    id
    placeholder
    }
    }
    ...
    if (existingData.anaemiaAndCkdImpactAssessments.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.anaemiaAndCkdImpactAssessments[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAnaemiaAndCkdImpactAssessment.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchAnaemiaAndCkdImpactAssessmentData = async () => {
      const query = gql`
        query {
            anaemiaAndCkdImpactAssessments {
            id
      ...
      try {
        ...
        console.log(getData.anaemiaAndCkdImpactAssessments[0]);
        setQuestionsToDisplay(getData.anaemiaAndCkdImpactAssessments)
   ...
   fetchAnaemiaAndCkdImpactAssessmentData();
  }, [dataPosted]);
...
export default AnaemiaAndCkdImpactAssessment