...
const BreastCancerThree = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteBreastCancerThree($id: ID!) {
    deleteBreastCancerThree(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createBreastCancerThree($data: BreastCancerThreeCreateInput!) {
    createBreastCancerThree(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishBreastCancerThree($id: ID!) {
    publishBreastCancerThree(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createBreastCancerThree.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingBreastCancerThrees {
  breastCancerThrees {
    id
    placeholder
    }
    ...
    if (existingData.breastCancerThrees.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.breastCancerThrees[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishBreastCancerThree.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchBreastCancerThreeData = async () => {
      const query = gql`
        query {
            breastCancerThrees {
            id
      ...
      try {
        ...
        console.log(getData.breastCancerThrees[0]);
        setQuestionsToDisplay(getData.breastCancerThrees)
...
export default BreastCancerThree