...
const HypoalbuminemiaCardiovascular = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteHypoalbuminemiaCardiovascular($id: ID!) {
    deleteHypoalbuminemiaCardiovascular(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHypoalbuminemiaCardiovascular($data: HypoalbuminemiaCardiovascularCreateInput!) {
    createHypoalbuminemiaCardiovascular(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishHypoalbuminemiaCardiovascular($id: ID!) {
    publishHypoalbuminemiaCardiovascular(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHypoalbuminemiaCardiovascular.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingHypoalbuminemiaCardiovasculars {
   hypoalbuminemiaCardiovasculars {
    id
    placeholder
    }
    }
    ...
    if (existingData.hypoalbuminemiaCardiovasculars.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.hypoalbuminemiaCardiovasculars[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHypoalbuminemiaCardiovascular.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchHypoalbuminemiaCardiovascularData = async () => {
      const query = gql`
        query {
            hypoalbuminemiaCardiovasculars {
            id
      ...
      try {
        ...
        console.log(getData.hypoalbuminemiaCardiovasculars[0]);
        setQuestionsToDisplay(getData.hypoalbuminemiaCardiovasculars)
   ...
   fetchHypoalbuminemiaCardiovascularData();
  }, [dataPosted]);
...
export default HypoalbuminemiaCardiovascular
