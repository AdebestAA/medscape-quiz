...
const PnhCaseOne = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deletePnhCaseOne($id: ID!) {
    deletePnhCaseOne(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createPnhCaseOne($data: PnhCaseOneCreateInput!) {
    createPnhCaseOne(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishPnhCaseOne($id: ID!) {
    publishPnhCaseOne(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createPnhCaseOne.id;
...
const overwriteAndPublishQuest = async (newData) => {
  ...
  pnhCaseOnes {
    ...
    if (existingData.pnhCaseOnes.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.pnhCaseOnes[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishPnhCaseOne.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchPnhCaseOneData = async () => {
      const query = gql`
        query {
            pnhCaseOnes {
            id
      ...
      try {
        ...
        console.log(getData.pnhCaseOnes[0]);
        setQuestionsToDisplay(getData.pnhCaseOnes)
