...
const HemophiliaCaseOne = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteHemophiliaCaseOne($id: ID!) {
    deleteHemophiliaCaseOne(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHemophiliaCaseOne($data: HemophiliaCaseOneCreateInput!) {
    createHemophiliaCaseOne(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishHemophiliaCaseOne($id: ID!) {
    publishHemophiliaCaseOne(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHemophiliaCaseOne.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingHemophiliaCaseOnes {
   hemophiliaCaseOnes {
    id
    placeholder
    }
    ...
    if (existingData.hemophiliaCaseOnes.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.hemophiliaCaseOnes[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHemophiliaCaseOne.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchHemophiliaCaseOneData = async () => {
      const query = gql`
        query {
            hemophiliaCaseOnes {
            id
      ...
      try {
        ...
        console.log(getData.hemophiliaCaseOnes[0]);
        setQuestionsToDisplay(getData.hemophiliaCaseOnes)
...
export default HemophiliaCaseOne