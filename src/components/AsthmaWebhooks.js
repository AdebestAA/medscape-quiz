...
const AsthmaWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAsthmaWebhook($id: ID!) {
    deleteAsthmaWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAsthmaWebhook($data: AsthmaWebhookCreateInput!) {
    createAsthmaWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAsthmaWebhook($id: ID!) {
    publishAsthmaWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAsthmaWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingAsthmaWebhooks {
   asthmaWebhooks {
    id
    placeholder
    }
    ...
    if (existingData.asthmaWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.asthmaWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAsthmaWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchAsthmaWebhookData = async () => {
      const query = gql`
        query {
           asthmaWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.asthmaWebhooks[0]);
        setQuestionsToDisplay(getData.asthmaWebhooks)
