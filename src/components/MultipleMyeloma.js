...
const MultipleMyeloma = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteMultipleMyeloma($id: ID!) {
    deleteMultipleMyeloma(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createMultipleMyeloma($data: MultipleMyelomaCreateInput!) {
    createMultipleMyeloma(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishMultipleMyeloma($id: ID!) {
    publishMultipleMyeloma(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createMultipleMyeloma.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingMultipleMyelomas {
   multipleMyelomas {
    id
    placeholder
    }
    ...
    if (existingData.multipleMyelomas.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.multipleMyelomas[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishMultipleMyeloma.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchMultipleMyelomaData = async () => {
      const query = gql`
        query {
            multipleMyelomas {
            id
      ...
      try {
        ...
        console.log(getData.multipleMyelomas[0]);
        setQuestionsToDisplay(getData.multipleMyelomas)
