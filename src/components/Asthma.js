...
console.log(dataFromTypeForm);
 ...
 mutation deleteAsthma($id: ID!) {
    deleteAsthma(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAsthma($data: AsthmaCreateInput!) {
    createAsthma(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAsthma($id: ID!) {
    publishAsthma(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAsthma.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   asthmas {
    ...
    if (existingData.asthmas.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.asthmas[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAsthma.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchAsthmaData = async () => {
      ...
      try {
        ...
        console.log(getData.asthmas[0]);
        setQuestionsToDisplay(getData.asthmas)
