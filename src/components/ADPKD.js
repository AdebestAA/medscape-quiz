...
console.log(dataFromTypeForm);
 ...
 mutation deleteAdpkd($id: ID!) {
    deleteAdpkd(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAdpkd($data: AdpkdCreateInput!) {
    createAdpkd(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAdpkd($id: ID!) {
    publishAdpkd(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   adpkds {
    ...
    if (existingData.adpkds.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.adpkds[0].id;
  ...
  placeholder: newData
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAdpkd.id;
    console.log('New quest created:', newQuest);
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAdpkd.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchAdpkdData = async () => {
      ...
      try {
        ...
        console.log(getData.adpkds[0]);
        setQuestionsToDisplay(getData.adpkds)
