...
const MyastheniaGravis = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteMyastheniaGravis($id: ID!) {
    deleteMyastheniaGravis(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createMyastheniaGravis($data: MyastheniaGravisCreateInput!) {
    createMyastheniaGravis(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishMyastheniaGravis($id: ID!) {
    publishMyastheniaGravis(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createMyastheniaGravis.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingMyastheniaGraviss {
    myastheniaGraviss {
    id
    placeholder
    }
    ...
    if (existingData.myastheniaGraviss.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.myastheniaGraviss[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishMyastheniaGravis.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchMyastheniaGravisData = async () => {
      const query = gql`
        query {
            myastheniaGraviss {
            id
      ...
      try {
        ...
        console.log(getData.myastheniaGraviss[0]);
        setQuestionsToDisplay(getData.myastheniaGraviss)
