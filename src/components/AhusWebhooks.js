...
const AhusWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAhusWebhook($id: ID!) {
    deleteAhusWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAhusWebhook($data: AhusWebhookCreateInput!) {
    createAhusWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAhusWebhook($id: ID!) {
    publishAhusWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAhusWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   ahusWebhooks {
    ...
    if (existingData.ahusWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.ahusWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAhusWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchAhusWebhookData = async () => {
      const query = gql`
        query {
            ahusWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.ahusWebhooks[0]);
        setQuestionsToDisplay(getData.ahusWebhooks)
