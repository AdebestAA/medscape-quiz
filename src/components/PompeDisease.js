...
const PompeDisease = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deletePompeDisease($id: ID!) {
    deletePompeDisease(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createPompeDisease($data: PompeDiseaseCreateInput!) {
    createPompeDisease(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishPompeDisease($id: ID!) {
    publishPompeDisease(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createPompeDisease.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingPompeDiseases {
   pompeDiseases {
    id
    placeholder
    }
    ...
    if (existingData.pompeDiseases.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.pompeDiseases[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishPompeDisease.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchPompeDiseaseData = async () => {
      const query = gql`
        query {
            pompeDiseases {
            id
      ...
      try {
        ...
        console.log(getData.pompeDiseases[0]);
        setQuestionsToDisplay(getData.pompeDiseases)
