...
const LymphomaCaseOne = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLymphomaCaseOne($id: ID!) {
    deleteLymphomaCaseOne(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLymphomaCaseOne($data: LymphomaCaseOneCreateInput!) {
    createLymphomaCaseOne(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLymphomaCaseOne($id: ID!) {
    publishLymphomaCaseOne(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLymphomaCaseOne.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingLymphomaCaseOnes {
   lymphomaCaseOnes {
    id
    placeholder
    }
    ...
    if (existingData.lymphomaCaseOnes.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.lymphomaCaseOnes[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLymphomaCaseOne.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchLymphomaCaseOneData = async () => {
      const query = gql`
        query {
            lymphomaCaseOnes {
            id
      ...
      try {
        ...
        console.log(getData.lymphomaCaseOnes[0]);
        setQuestionsToDisplay(getData.lymphomaCaseOnes)
...
export default LymphomaCaseOne
