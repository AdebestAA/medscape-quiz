...
console.log(dataFromTypeForm);
 ...
 mutation deleteHfaid($id: ID!) {
    deleteHfaid(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHfaid($data: HfaidCreateInput!) {
    createHfaid(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishHfaid($id: ID!) {
    publishHfaid(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHfaid.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   hfaids {
    ...
    if (existingData.hfaids.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.hfaids[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHfaid.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchHfaidData = async () => {
      ...
      try {
        ...
        console.log(getData.hfaids[0]);
        setQuestionsToDisplay(getData.hfaids)
