...
const IganCaseTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteIganCaseTwo($id: ID!) {
    deleteIganCaseTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createIganCaseTwo($data: IganCaseTwoCreateInput!) {
    createIganCaseTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishIganCaseTwo($id: ID!) {
    publishIganCaseTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createIganCaseTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  ...
  iganCaseTwos {
    ...
    if (existingData.iganCaseTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.iganCaseTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishIganCaseTwo.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchIganCaseTwoData = async () => {
      const query = gql`
        query {
            iganCaseTwos {
            id
      ...
      try {
        ...
        console.log(getData.iganCaseTwos[0]);
        setQuestionsToDisplay(getData.iganCaseTwos)
