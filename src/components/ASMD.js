...
console.log(dataFromTypeForm);
 ...
 mutation deleteAsmd($id: ID!) {
    deleteAsmd(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAsmd($data: AsmdCreateInput!) {
    createAsmd(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAsmd($id: ID!) {
    publishAsmd(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAsmd.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   asmds {
    ...
    if (existingData.asmds.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.asmds[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAsmd.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchAsmdData = async () => {
      ...
      try {
        ...
        console.log(getData.asmds[0]);
        setQuestionsToDisplay(getData.asmds)
