...
const NZeroFiveM = ({dataFromTypeForm}) => {
...
overwriteAndPublishQuest(dataFromTypeForm);
 ...
 mutation deleteNZeroFiveM($id: ID!) {
    deleteNZeroFiveM(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createNZeroFiveM($data: NZeroFiveMCreateInput!) {
    createNZeroFiveM(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishNZeroFiveM($id: ID!) {
    publishNZeroFiveM(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createNZeroFiveM.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   nZeroFiveMs {
    ...
    if (existingData.nZeroFiveMs.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.nZeroFiveMs[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishNZeroFiveM.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchNZeroFiveMData = async () => {
      const query = gql`
        query {
            nZeroFiveMs {
            id
      ...
      try {
        ...
        console.log(getData.nZeroFiveMs[0]);
        setQuestionsToDisplay(getData.nZeroFiveMs)
