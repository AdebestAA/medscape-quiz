...
const HypertrophicCardiomyopathyCaseOne = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteHypertrophicCardiomyopathyCaseOne($id: ID!) {
    deleteHypertrophicCardiomyopathyCaseOne(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHypertrophicCardiomyopathyCaseOne($data: HypertrophicCardiomyopathyCaseOneCreateInput!) {
    createHypertrophicCardiomyopathyCaseOne(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishHypertrophicCardiomyopathyCaseOne($id: ID!) {
    publishHypertrophicCardiomyopathyCaseOne(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHypertrophicCardiomyopathyCaseOne.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingHypertrophicCardiomyopathyCaseOnes {
  hypertrophicCardiomyopathyCaseOnes {
    id
    placeholder
    }
    }
    ...
    if (existingData.hypertrophicCardiomyopathyCaseOnes.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.hypertrophicCardiomyopathyCaseOnes[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHypertrophicCardiomyopathyCaseOne.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchHypertrophicCardiomyopathyCaseOneData = async () => {
      const query = gql`
        query {
            hypertrophicCardiomyopathyCaseOnes {
            id
      ...
      try {
        ...
        console.log(getData.hypertrophicCardiomyopathyCaseOnes[0]);
        setQuestionsToDisplay(getData.hypertrophicCardiomyopathyCaseOnes)
   ...
   fetchHypertrophicCardiomyopathyCaseOneData();
  }, [dataPosted]);
...
export default HypertrophicCardiomyopathyCaseOne