...
const ItpWebhooks = ({dataFromTypeForm}) => {
...
overwriteAndPublishQuest(dataFromTypeForm);
 ...
 mutation deleteItpWebhook($id: ID!) {
    deleteItpWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createItpWebhook($data: ItpWebhookCreateInput!) {
    createItpWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishItpWebhook($id: ID!) {
    publishItpWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createItpWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   itpWebhooks {
    ...
    if (existingData.itpWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.itpWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishItpWebhook.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchMgHitData = async () => {
      ...
      try {
        ...
        console.log(getData.itpWebhooks[0]);
        setQuestionsToDisplay(getData.itpWebhooks)
...
<article className="w-[50%] text-2xl" key={welItem.id}>
{welItem.title.split("\n").map((titleItem,index)=>{
    if (index === 0) {
        return (
...
return (
<h1 key={index} className="font-extrabold text-center">
    {titleItem.replace("*", "")}
</h1>
)

})}
<span className="text-sm">{welItem.properties.description}</span>
...
// CONTACT INFO
 ...
 return (
    ...
    <aside ref={el => eachQuestAndFeedback.current[index] = el} key={quest?.id} className=" px-64 py-16 text-lg flex flex-col items-center justify-center  h-screen overflow-y-scroll space-y-4 ">
        ...
        <p>{quest.properties.description?.split("*").map((desc,descIndex)=>{
