...
const IganWebhooksTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteIganWebhooksTwo($id: ID!) {
    deleteIganWebhooksTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createIganWebhooksTwo($data: IganWebhooksTwoCreateInput!) {
    createIganWebhooksTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishIganWebhooksTwo($id: ID!) {
    publishIganWebhooksTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createIganWebhooksTwo.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingIganWebhooksTwos {
   iganWebhooksTwos {
    id
    placeholder
    }
    ...
    if (existingData.iganWebhooksTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.iganWebhooksTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishIganWebhooksTwo.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchIganWebhooksTwoData = async () => {
      const query = gql`
        query {
           iganWebhooksTwos {
            id
      ...
      try {
        ...
        console.log(getData.iganWebhooksTwos[0]);
        setQuestionsToDisplay(getData.iganWebhooksTwos)
...
export default IganWebhooksTwo