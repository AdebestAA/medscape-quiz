...
console.log(dataFromTypeForm);
 ...
 mutation deleteCopd($id: ID!) {
    deleteCopd(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCopd($data: CopdCreateInput!) {
    createCopd(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCopd($id: ID!) {
    publishCopd(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCopd.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   copds {
    ...
    if (existingData.copds.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.copds[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCopd.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchCopdData = async () => {
      ...
      try {
        ...
        console.log(getData.copds[0]);
        setQuestionsToDisplay(getData.copds)
