...
const CytomegalovirusPreTest = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCytomegalovirusPreTest($id: ID!) {
    deleteCytomegalovirusPreTest(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCytomegalovirusPreTest($data: CytomegalovirusPreTestCreateInput!) {
    createCytomegalovirusPreTest(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCytomegalovirusPreTest($id: ID!) {
    publishCytomegalovirusPreTest(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCytomegalovirusPreTest.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCytomegalovirusPreTests {
  cytomegalovirusPreTests {
    id
    placeholder
    }
    }
    ...
    if (existingData.cytomegalovirusPreTests.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.cytomegalovirusPreTests[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCytomegalovirusPreTest.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCytomegalovirusPreTestData = async () => {
      const query = gql`
        query {
            cytomegalovirusPreTests {
            id
      ...
      try {
        ...
        console.log(getData.cytomegalovirusPreTests[0]);
        setQuestionsToDisplay(getData.cytomegalovirusPreTests)
   ...
   fetchCytomegalovirusPreTestData();
  }, [dataPosted]);
...
export default CytomegalovirusPreTest