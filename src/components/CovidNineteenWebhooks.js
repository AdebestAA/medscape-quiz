...
const CovidNineteenWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCovidNineteenWebhook($id: ID!) {
    deleteCovidNineteenWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCovidNineteenWebhook($data: CovidNineteenWebhookCreateInput!) {
    createCovidNineteenWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCovidNineteenWebhook($id: ID!) {
    publishCovidNineteenWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCovidNineteenWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCovidNineteenWebhooks {
   covidNineteenWebhooks {
    ...
    if (existingData.covidNineteenWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.covidNineteenWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCovidNineteenWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCovidNineteenWebhookData = async () => {
      const query = gql`
        query {
            covidNineteenWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.covidNineteenWebhooks[0]);
        setQuestionsToDisplay(getData.covidNineteenWebhooks)
...
export default CovidNineteenWebhooks