...
const PnhCaseTwo = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deletePnhCaseTwo($id: ID!) {
    deletePnhCaseTwo(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createPnhCaseTwo($data: PnhCaseTwoCreateInput!) {
    createPnhCaseTwo(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishPnhCaseTwo($id: ID!) {
    publishPnhCaseTwo(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
const overwriteAndPublishQuest = async (newData) => {
  ...
  pnhCaseTwos {
    ...
    if (existingData.pnhCaseTwos.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.pnhCaseTwos[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishPnhCaseTwo.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchPnhCaseTwoData = async () => {
      const query = gql`
        query {
            pnhCaseTwos {
            id
      ...
      try {
        ...
        console.log(getData.pnhCaseTwos[0]);
        setQuestionsToDisplay(getData.pnhCaseTwos)
