...
const NDyspnea = ({dataFromTypeForm}) => {
...
overwriteAndPublishQuest(dataFromTypeForm);
 ...
 mutation deleteNDyspnea($id: ID!) {
    deleteNDyspnea(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createNDyspnea($data: NDyspneaCreateInput!) {
    createNDyspnea(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishNDyspnea($id: ID!) {
    publishNDyspnea(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createNDyspnea.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   nDyspneas {
    ...
    if (existingData.nDyspneas.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.nDyspneas[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishNDyspnea.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchNDyspneaData = async () => {
      ...
      try {
        ...
        console.log(getData.nDyspneas[0]);
        setQuestionsToDisplay(getData.nDyspneas)
