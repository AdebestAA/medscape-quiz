...
const Leukemia = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLeukemia($id: ID!) {
    deleteLeukemia(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLeukemia($data: LeukemiaCreateInput!) {
    createLeukemia(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLeukemia($id: ID!) {
    publishLeukemia(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLeukemia.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   leukemias {
    ...
    if (existingData.leukemias.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.leukemias[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLeukemia.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchLeukemiaData = async () => {
      ...
      try {
        ...
        console.log(getData.leukemias[0]);
        setQuestionsToDisplay(getData.leukemias)
