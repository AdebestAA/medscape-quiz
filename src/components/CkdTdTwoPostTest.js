...
const CkdTdTwoPostTest = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCkdTdTwoPostTest($id: ID!) {
    deleteCkdTdTwoPostTest(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCkdTdTwoPostTest($data: CkdTdTwoPostTestCreateInput!) {
    createCkdTdTwoPostTest(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCkdTdTwoPostTest($id: ID!) {
    publishCkdTdTwoPostTest(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCkdTdTwoPostTest.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCkdTdTwoPostTests {
  ckdTdTwoPostTests {
    id
    placeholder
    }
    ...
    if (existingData.ckdTdTwoPostTests.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.ckdTdTwoPostTests[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCkdTdTwoPostTest.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCkdTdTwoPostTestData = async () => {
      const query = gql`
        query {
            ckdTdTwoPostTests {
            id
      ...
      try {
        ...
        console.log(getData.ckdTdTwoPostTests[0]);
        setQuestionsToDisplay(getData.ckdTdTwoPostTests)
...
export default CkdTdTwoPostTest