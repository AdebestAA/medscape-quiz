...
const Hemophilia = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteHemophilia($id: ID!) {
    deleteHemophilia(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHemophilia($data: HemophiliaCreateInput!) {
    createHemophilia(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishHemophilia($id: ID!) {
    publishHemophilia(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHemophilia.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   hemophilias {
    ...
    if (existingData.hemophilias.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.hemophilias[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHemophilia.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchHemophiliaData = async () => {
      const query = gql`
        query {
            hemophilias {
            id
      ...
      try {
        ...
        console.log(getData.hemophilias[0]);
        setQuestionsToDisplay(getData.hemophilias)
