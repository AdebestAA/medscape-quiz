...
const DiabeticRetinopathyWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteDiabeticRetinopathyWebhook($id: ID!) {
    deleteDiabeticRetinopathyWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createDiabeticRetinopathyWebhook($data: DiabeticRetinopathyWebhookCreateInput!) {
    createDiabeticRetinopathyWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishDiabeticRetinopathyWebhook($id: ID!) {
    publishDiabeticRetinopathyWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createDiabeticRetinopathyWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingDiabeticRetinopathyWebhooks {
   diabeticRetinopathyWebhooks {
    id
    placeholder
    }
    }
    ...
    if (existingData.diabeticRetinopathyWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.diabeticRetinopathyWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishDiabeticRetinopathyWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchDiabeticRetinopathyWebhookData = async () => {
      const query = gql`
        query {
            diabeticRetinopathyWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.diabeticRetinopathyWebhooks[0]);
        setQuestionsToDisplay(getData.diabeticRetinopathyWebhooks)
   ...
   fetchDiabeticRetinopathyWebhookData();
  }, [dataPosted]);
...
export default DiabeticRetinopathyWebhooks