...
const NsclcWebhooks = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteNsclcWebhook($id: ID!) {
    deleteNsclcWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createNsclcWebhook($data: NsclcWebhookCreateInput!) {
    createNsclcWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishNsclcWebhook($id: ID!) {
    publishNsclcWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createNsclcWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingNsclcWebhooks {
   nsclcWebhooks {
    id
    placeholder
    }
    ...
    if (existingData.nsclcWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.nsclcWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishNsclcWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchNsclcWebhookData = async () => {
      const query = gql`
        query {
            nsclcWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.nsclcWebhooks[0]);
        setQuestionsToDisplay(getData.nsclcWebhooks)
