...
const AcuteMyeloidLeukaemia = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteAcuteMyeloidLeukaemia($id: ID!) {
    deleteAcuteMyeloidLeukaemia(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createAcuteMyeloidLeukaemia($data: AcuteMyeloidLeukaemiaCreateInput!) {
    createAcuteMyeloidLeukaemia(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishAcuteMyeloidLeukaemia($id: ID!) {
    publishAcuteMyeloidLeukaemia(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createAcuteMyeloidLeukaemia.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingAcuteMyeloidLeukaemias {
  acuteMyeloidLeukaemias {
    ...
    if (existingData.acuteMyeloidLeukaemias.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.acuteMyeloidLeukaemias[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishAcuteMyeloidLeukaemia.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchAcuteMyeloidLeukaemiaData = async () => {
      const query = gql`
        query {
            acuteMyeloidLeukaemias {
            id
      ...
      try {
        ...
        console.log(getData.acuteMyeloidLeukaemias[0]);
        setQuestionsToDisplay(getData.acuteMyeloidLeukaemias)
   ...
   fetchAcuteMyeloidLeukaemiaData();
  }, [dataPosted]);
...
export default AcuteMyeloidLeukaemia