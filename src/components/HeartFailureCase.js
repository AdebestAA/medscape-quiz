...
const HeartFailureCase = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteHeartFailureCase($id: ID!) {
    deleteHeartFailureCase(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHeartFailureCase($data: HeartFailureCaseCreateInput!) {
    createHeartFailureCase(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishHeartFailureCase($id: ID!) {
    publishHeartFailureCase(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHeartFailureCase.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingHeartFailureCases {
  heartFailureCases {
    id
    placeholder
    }
    ...
    if (existingData.heartFailureCases.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.heartFailureCases[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHeartFailureCase.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchHeartFailureCaseData = async () => {
      const query = gql`
        query {
            heartFailureCases {
            id
      ...
      try {
        ...
        console.log(getData.heartFailureCases[0]);
        setQuestionsToDisplay(getData.heartFailureCases)
...
export default HeartFailureCase