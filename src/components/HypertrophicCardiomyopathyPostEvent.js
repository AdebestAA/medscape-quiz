...
const HypertrophicCardiomyopathyPostEvent = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteHypertrophicCardiomyopathyPostEvent($id: ID!) {
    deleteHypertrophicCardiomyopathyPostEvent(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createHypertrophicCardiomyopathyPostEvent($data: HypertrophicCardiomyopathyPostEventCreateInput!) {
    createHypertrophicCardiomyopathyPostEvent(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishHypertrophicCardiomyopathyPostEvent($id: ID!) {
    publishHypertrophicCardiomyopathyPostEvent(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createHypertrophicCardiomyopathyPostEvent.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingHypertrophicCardiomyopathyPostEvents {
  hypertrophicCardiomyopathyPostEvents {
    id
    placeholder
    }
    }
    ...
    if (existingData.hypertrophicCardiomyopathyPostEvents.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.hypertrophicCardiomyopathyPostEvents[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishHypertrophicCardiomyopathyPostEvent.id) {
      return true
    }
    else{
      return false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchHypertrophicCardiomyopathyPostEventData = async () => {
      const query = gql`
        query {
            hypertrophicCardiomyopathyPostEvents {
            id
      ...
      try {
        ...
        console.log(getData.hypertrophicCardiomyopathyPostEvents[0]);
        setQuestionsToDisplay(getData.hypertrophicCardiomyopathyPostEvents)
   ...
   fetchHypertrophicCardiomyopathyPostEventData();
  }, [dataPosted]);
...
export default HypertrophicCardiomyopathyPostEvent