...
const SkinCancerGeoblock = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteSkinCancerGeoblock($id: ID!) {
    deleteSkinCancerGeoblock(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createSkinCancerGeoblock($data: SkinCancerGeoblockCreateInput!) {
    createSkinCancerGeoblock(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishSkinCancerGeoblock($id: ID!) {
    publishSkinCancerGeoblock(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createSkinCancerGeoblock.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingSkinCancerGeoblocks {
   skinCancerGeoblocks {
    id
    placeholder
    }
    ...
    if (existingData.skinCancerGeoblocks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.skinCancerGeoblocks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishSkinCancerGeoblock.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchSkinCancerGeoblockData = async () => {
      const query = gql`
        query {
            skinCancerGeoblocks {
            id
      ...
      try {
        ...
        console.log(getData.skinCancerGeoblocks[0]);
        setQuestionsToDisplay(getData.skinCancerGeoblocks)
...
{quest?.properties?.choices?.map((choice,choiceIndex)=>{
    ...
    return (
        ...
        <aside key={choice.id} className={`my-2 ${choice.label.length  < 50 ? " w-[30%] mx-[2px] " : " w-[100%]"}  rounded-sm text-start text-[0.95rem] border-[2px] border-[navy] flex items-start px-2 py-2 gap-x-2 bg-[#D7DDE8] hover:bg-[#A6BCDA]  cursor-pointer`}
        onClick={()=>{
            if (step + 1 == questionsToDisplay[0]?.placeholder?.fields?.length) {
                return
            }
...
export default SkinCancerGeoblock
