...
const Lymphoma = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteLymphoma($id: ID!) {
    deleteLymphoma(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createLymphoma($data: LymphomaCreateInput!) {
    createLymphoma(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishLymphoma($id: ID!) {
    publishLymphoma(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createLymphoma.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   lymphomas {
    ...
    if (existingData.lymphomas.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.lymphomas[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishLymphoma.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchLymphomaData = async () => {
      ...
      try {
        ...
        console.log(getData.lymphomas[0]);
        setQuestionsToDisplay(getData.lymphomas)
