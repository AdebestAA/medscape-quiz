...
const CytomegalovirusWebhook = ({dataFromTypeForm}) => {
...
console.log(dataFromTypeForm);
 ...
 mutation deleteCytomegalovirusWebhook($id: ID!) {
    deleteCytomegalovirusWebhook(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createCytomegalovirusWebhook($data: CytomegalovirusWebhookCreateInput!) {
    createCytomegalovirusWebhook(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishCytomegalovirusWebhook($id: ID!) {
    publishCytomegalovirusWebhook(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createCytomegalovirusWebhook.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
    ...
    query getExistingCytomegalovirusWebhooks {
   cytomegalovirusWebhooks {
    id
    placeholder
    }
    }
    ...
    if (existingData.cytomegalovirusWebhooks.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.cytomegalovirusWebhooks[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishCytomegalovirusWebhook.id) {
      return true
    }
    else{
      false
...
console.log(dataFromTypeForm);
  ...
  useEffect(() => {
    ...
    const fetchCytomegalovirusWebhookData = async () => {
      const query = gql`
        query {
            cytomegalovirusWebhooks {
            id
      ...
      try {
        ...
        console.log(getData.cytomegalovirusWebhooks[0]);
        setQuestionsToDisplay(getData.cytomegalovirusWebhooks)
   ...
   fetchCytomegalovirusWebhookData();
  }, [dataPosted]);
...
export default CytomegalovirusWebhook
