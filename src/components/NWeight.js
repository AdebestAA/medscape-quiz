...
const NWeight = ({dataFromTypeForm}) => {
...
overwriteAndPublishQuest(dataFromTypeForm);
 ...
 mutation deleteNWeight($id: ID!) {
    deleteNWeight(where: { id: $id }) {
      id
    }
  }
`;
...
const createQuestMutation = gql`
 mutation createNWeight($data: NWeightCreateInput!) {
    createNWeight(data: $data) {
      id
      placeholder
    }
  }
...
const publishQuestMutation = gql`
  mutation publishNWeight($id: ID!) {
    publishNWeight(where: { id: $id }) {
      id
      placeholder
      publishedAt
    }
...
async function tryT() {
  ...
  placeholder: {name:"james"}
    ...
    //   fields: newData,
    // });
    const questId = newQuest.createNWeight.id;
...
const overwriteAndPublishQuest = async (newData) => {
  try {
   ...
   nWeights {
    ...
    if (existingData.nWeights.length > 0) {
      // Step 2: Delete the existing quest
      const questId = existingData.nWeights[0].id;
  ...
  placeholder: newData
   ...
   setDataPosted(()=> {
    if (publishedQuest.publishNWeight.id) {
      return true
    }
    else{
      false
  ...
  useEffect(() => {
    ...
    const fetchNWeightData = async () => {
      ...
      try {
        ...
        console.log(getData.nWeights[0]);
        setQuestionsToDisplay(getData.nWeights)
